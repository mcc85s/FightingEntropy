...
\\  Date       : 2024-01-21 18:37:25                                                                  //
 \\==================================================================================================// 
    ...
    TODO       : Launches the (graphical user interface/control) for:
                 [FightingEntropy()][Company Information Management Database]
...
Function Invoke-cimdb
{
    ...
    Class cimdbXaml
    {
        ...
        '                <Image Source="C:\ProgramData\Secure Digits Plus LLC\FightingEntropy\2024.1.0\Graphics\sdplogo.png"/>',
        '            </Button>',
        ...
        '                                            <Image Source="C:\ProgramData\Secure Digits Plus LLC\FightingEntropy\2024.1.0\Control\up.png"/>',
        '                                        </Button>',
        '                                        <Button Grid.Row="4"',
        '                                                    Name="EditClientPhoneMoveDown">',
        '                                            <Image Source="C:\ProgramData\Secure Digits Plus LLC\FightingEntropy\2024.1.0\Control\down.png"/>',
        '                                        </Button>',
        '                                    </Grid>',
        '                                </Grid>',
        '                            </Grid>',
    ...
    Class cimdbFolderFile
    {
        [String]   $Source
        [String]     $Name
        [String] $Fullname
        cimdbFolderFile([String]$Source,[String]$Fullname)
        {
            $This.Source   = $Source
            $This.Name     = Split-Path $Fullname -Leaf
    ...
    Class cimdbFolderItem
    {
        ...
        [UInt32]    $Total
        [Object]     $Item
        cimdbFolderItem([UInt32]$Index,[String]$Fullname)
        {
            $This.Index    = $Index
            $This.Name     = Split-Path $Fullname -Leaf
            $This.Fullname = $Fullname
        }
        [Object] cimdbFolderFile([String]$Source,[String]$Fullname)
        {
            Return [cimdbFolderFile]::New($Source,$Fullname)
        }
        GetChildItem()
        {
            $Hash       = @{ }
            ForEach ($File in [System.IO.Directory]::GetFileSystemEntries($This.Fullname))
            {
                $Hash.Add($Hash.Count,$This.cimdbFolderFile($This.Name,$File))
            }

            $This.Item  = Switch ($Hash.Count)
            {
                0 { $Null } 1 { $Hash[0] } Default { $Hash[0..($Hash.Count-1)] }
            }

            $This.Total = $This.Item.Count
        }
        [String] ToString()
        {
            Return $This.Name
    ...
    Class cimdbFolderController
    {
        [String]   $Path
        [Object] $Output
        cimdbFolderController([String]$Path)
        {
        ...
        [Object] cimdbFolderItem([UInt32]$Index,[String]$Fullname)
        {
            Return [cimdbFolderItem]::New($Index,$Fullname)
        }
        [Object] New([String]$Name)
        {
            Return $This.cimdbFolderItem($This.Output.Count,$This.Name($Name))
        }
        [String] Name([String]$Name)
        {
            Return "{0}\{1}" -f $This.Path, $Name
        }
        Refresh()
        {
            $This.Clear()
            ...
            $Hash = @{ }
            ForEach ($Name in [System.Enum]::GetNames([cimdbFolderType]))
            {
                $Item        = $This.New($Name)
                $Item.Index  = $Hash.Count
                $Item.Exists = [System.IO.Directory]::Exists($Item.Fullname)

                If (!$Item.Exists)
                {
                    [System.IO.Directory]::CreateDirectory($Item.Fullname)
                    $Item.Exists ++
                }
        ...
        [String] ToString()
        {
            Return "<FEModule.cimdb.Folder[Controller]>"
        }
    }
    ...
    Class cimdbUid
    {
        ...
        Hidden [Int32]      $Mode
        [Object]          $Record
        cimdbUid([UInt32]$Index,[UInt32]$Slot)
        {
            $This.Index    = $Index
            $This.DateTime = $This.GetDateTime()
            $Dt            = $This.DateTime -Split " "
            $This.Date     = $Dt[0]
            $This.Time     = $Dt[1]
        ...
        cimdbUid([Switch]$Flags,[Object]$Uid)
        {
            $This.Index    = $Uid.Index
            $This.DateTime = "{0} {1}" -f $Uid.Date, $Uid.Time
            $This.Date     = $Uid.Date
            $This.Time     = $Uid.Time
            $This.Uid      = $Uid.Uid
            $This.Slot     = $Uid.Slot

            $This.Record   = $Uid.Record
        }
        ...
        [String] Filename()
        {
            Return ([DateTime]$This.DateTime).ToString("yyyyMMddHHmmssfff")
        }
        [String] ToString()
        {
    ...
    Class cimdbDatabaseController
    {
        ...
        [Object]     $History
        [Object]      $Output
        cimdbDatabaseController([String]$Path,[Object]$Slot)
        {
            $This.Slot    = $Slot
            $This.Root    = $This.cimdbFolderController($Path)
            $This.Clear()
        }
        [Object] cimdbFolderController([String]$Path)
        {
            Return [cimdbFolderController]::New($Path)
        }
        Clear()
        {
            $This.History = @( )
        ...
        [Object] cimdbUid([UInt32]$Index,[UInt32]$Slot)
        {
            Return [cimdbUid]::New($Index,$Slot)
        }
        [Object] cimdbUid([Switch]$Flags,[Object]$File)
        {
            Return [cimdbUid]::New($Flags,$This.Infile($File))
        }
        ...
        {
            Return $This.cimdbUid($This.Output.Count,$This.GetIndex($Name))
        }
        ...
        {
            Return @($This.Output | ? Slot -match $This.GetIndex($Name))
        }
        [Object] New([String]$Name)
        {
            $Uid        = $This.Entry($Name)
        ...
        [Object] Outfile([Object]$Uid)
        {
            Return $Uid | ConvertTo-Json -Depth 4
        }
        [Object] Infile([Object]$File)
        {
            Return [System.IO.File]::ReadAllLines($File.Fullname) | ConvertFrom-Json
        }
        Initialize()
        {
            <#
            $List = $Ctrl.Database.Root.Output | % Item

            ForEach ($Item in $List)
            {
                $Content = $Ctrl.Database.Infile($Item)
                $Ctrl.Database.cimdbUid($False,$Item)
            }
            #>
        }
        Save([Object]$Uid)
        {
            $Content  = $This.Outfile($Uid)
            $Parent   = $This.Root.Output | ? Name -eq $This.Slot.Category.Output[$Uid.Slot].Name
            $Target   = "{0}\{1}.txt" -f $Parent.Fullname, $Uid.Filename()
    ...
    Class cimdbController
    {
        ...
        [String] DatabasePath()
        {
            Return "{0}\{1}\cimdb" -f $This.Module.OS.Tx("Environment","ProgramFiles"), $This.Author()
        }
        ...
        [Object] Get([String]$Name)
        {
            ...
            Switch ($Name)
            {
                ...
                Database
                {
                    ...
                    $Item = [cimdbDatabaseController]::New($This.DatabasePath(),$This.Slot)
                }
                Current
                {
            ...
            Switch -Regex ($Ctrl.Current.Mode.Name)
            {
                ...
                Client
                {
                    ...
                    If ($Ctrl.Current.Validate.Pass)
                    {
                        ...
                        # // [Save the record]
                        $Ctrl.Database.Save($Ctrl.Current.Uid)
