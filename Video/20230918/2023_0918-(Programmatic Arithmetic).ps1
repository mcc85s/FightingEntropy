...
# // ====================================================================
# // | Provides (comparison/validation) between source and target paths |
# // ====================================================================

Class VideoFileValidation
{
    [Object] $Source
    [Object] $Target
    [UInt32] $Match
    VideoFileValidation([Object]$Source,[Object]$Target)
    {
        $This.Source = $Source
        $This.Target = $Target

        If ($This.Source.Size -match $This.Target.Size)
        {
            $This.Match ++
        }
    }
...
Class VideoFileTransfer
{
    ...
    [Object] VideoFileValidation([Object]$Source,[Object]$Target)
    {
        Return [VideoFileValidation]::New($Source,$Target)
    }
    ...
    Validation()
    {
        ...
        [Console]::WriteLine("Validating [~] ($($This.Source.Output.Count)) files")
        
        ForEach ($xSource in $This.Source.Output)
        {
            [Console]::WriteLine($xSource.Name)
        
            If ($xSource.Name -in $This.Target.Output.Name)
            {
                $xTarget = $This.Target.Output | ? Name -eq $xSource.Name
                If ($xTarget.Count -gt 1)
                {
                    Write-Error "Multiple files with the name: $($xSource.Name)"
        
                    $xTarget | % { 
                        
                        [Console]::WriteLine("Conflict [!] $($_.Fullname)") 
                        $Hash.Add($Hash.Count,$Ctrl.VideoFileValidation($xSource,$_))
                    }
                }
                Else
                {
                    $Hash.Add($Hash.Count,$This.VideoFileValidation($xSource,$xTarget))
                }
            }
        }
        
        $This.Validate = $Hash[0..($Hash.Count-1)]
    }
    Overwrite([Object]$Validate)
    {
        If ($Validate.GetType().Name -ne "VideoFileValidation")
        {
            Throw "Must provide a validation object"
        }

        [System.IO.File]::Delete($Validate.Target.Fullname)

        $This.VideoFileTransferStream($Validate.Source.Fullname,$Validate.Target.Fullname)
    }
    Force()
    {
        If (!$This.Validate)
        ...
        $Mismatch = $This.Validate | ? Match -eq 0
        If ($Mismatch.Count -gt 0)
        {
            ForEach ($File in $Mismatch)
            {
                $Line = "Replacing [~] Name: {0}, Size: {1}, Correct: {2}" -f $File.Source.Name,
                                                                              $File.Target.Size,
                                                                              $File.Source.Size
...
<# [Validation, for another lesson]

    $Ctrl.Validation()

    $Mismatch = $Ctrl.Validate | ? Match -eq 0
    If ($Mismatch.Count -gt 0)
    {
        ForEach ($File in $Mismatch)
        {
