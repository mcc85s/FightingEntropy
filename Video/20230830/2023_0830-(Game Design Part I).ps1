...
/¯¯¯¯¯¯¯¯¯¯¯¯¯¯                                                                                                         
    ...
    In order to design games, it is important to note what avenue of [game design] you wish to pursue, as there are
    multiple paths that can be taken (and are required), to [design a game]. Some of those avenues are:
    ...
    In this particular study, we will be focusing on a study sample called [Quake III Arena], which was developed by
     
    [id Software] and released in [December 1999].
    ...
    We won't be covering the history of [id Software], nor the number of games it has made that led to the creation
    of [Quake III Arena], however... as stated, the game pushed the limits of what computer games could do in many
    various ways.
    
    It is worth mentioning that the game retains quite a lot of charm and character that has prevailed over the
    course of the last (24) years.
                                                                                                         ______________/
\_______________________________________________________________________________________________________/ Introduction  
  Objective /¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯\
/¯¯¯¯¯¯¯¯¯¯¯                                                                                                            

    The objective of this document, is to cover the many things that go into creating a level for [Quake III Arena],
    
    as well as the necessary tools and skills to build a level with [replayability], and [competitiveness] to it.
    ...
    [GtkRadiant] is the level editor for [Quake III Arena], though I will be covering additional level design
    concepts with an alternative that is based on [GtkRadiant], called [NetRadiant].
    
    As for the link above, what I did NOT cover, was [HOW] gameplay mechanics are the reason why the game has
    retained its [charm] and [character] over the course of the last (24) years or so.
    
    Unlike its predecessors [Doom I + II], and [Quake I + II], the game can be configured to utilize custom 
    resolutions and high graphics settings that allow it to run in (HD/high definition) without modifying the game, 
    
    or its content.
    
    While there have been many games released by [id Software] since (1999) that have better: 
    __________________________________________________________
    | graphics | lighting | shading | animations | et cetera |
    ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
    ...every single one of them is based on a [very heavily modified version] of the [Q3A] engine, such as: 
    
    [+] [Return to Castle Wolfenstein, Enemy Territory, Wolfenstein, The New Order, The Old Blood, The New Colossus)
    ]
    [+] [Doom III + Ressurection of Evil]
    [+] [Rage I + II]
    [+] [Doom 2016 + Eternal]
    [+] [Quake IV + Champions]
    
    Given its age, the game runs relatively well on older hardware, and still retains quite a lot of chess-like
    characteristics. It's many modes of play range from:
    
    =========================================================================================
    | Free-for-all/Deathmatch | Tournament/1v1 | Team Deathmatch/TDM | Capture the flag/CTF |
    =========================================================================================
    
    There is plenty of debate on which mode is the [most popular] or the "best"...
    
    ...but make no mistake, its' [Tournament/1v1] mode cemented its position at a large number of yearly [QuakeCon]
    events, and is synonymous with [Quake Live]... since it is the [same exact game].
    
    It also resurfaced as a staple of [Quake Champions] with the advent of [Overwatch], [Fortnite], and 
    [Player Unknown Battlegrounds].
    
    To narrow the objective even further, we'll be focusing on [Tournament/1v1] mode, as well as how to make a reall
    y
    (cool + fun) level for it, that focuses on aspects of the [mode] and the [game], in order to produce something
    that is incredibly polished.
...
/¯¯¯¯¯¯¯¯¯¯                                                                                                             
    ...
    While the original [Quake III Arena] has a lot of tournament maps in it, [Quake III Team Arena] introduced a
    handful of maps that I think really illustrates [what to do], and [what NOT to do], in [designing levels] for
    ...
    \____________|_________________|____________|______________________________|________|________/
    
    So, allow me to explain each levels [strengths] and [weaknesses], and at some point, I will talk about each map
    in further detail, by providing commentary in a video that focuses on the [layout] and [item placement].
    
    =============================
    | House of Decay/mptourney1 |
    =============================
    
    This map is without a doubt, [extremely well made]. It is small enough to have a strong focus on (3) different
    weapons, and provides a really strong emphasis on [aesthetics] and [geometry] to allow the [gameplay] to swing
    in either direction.
    
    This map was remade for [Quake Live], and has other items added to it which take away from the charm of the 
    original. It probably goes without saying... but this is one of my favorite maps, above ALL of the original 
    [pro-q3dm/pro-q3tourney] maps. (...and that's saying something)
    ...
    While that's not exactly a bad thing, as it is common for a [game developer] to recycle [assets] and attempt
    to reuse those [assets] with a slightly different twist... in this instance, there are a lot of shortcomings to
    ...
    This forces the players to have to use [jump pads] to traverse from [tier] to [tier], which [exposes] a player
    to be [quite vulnerable] when they use them... which can be seen in the video link above.
    
    The [jump pads] is a serious sticking point, here... because this map would actually score a LOT higher as a
    [tournament] map if there were stairs on both sides to connect the [middle tier] to the [top tier], because
    at that point, the [multi-tiered combat] would be [far more cohesive] and a [lot less predictable].
    
    This point alone, though incredibly small... can be the [difference] between whether it [makes] or [breaks]
    a level. Simply put, [the map isn't badly made at all], but considering that it is named “mptourney2”... 
    ...
    However, the map [looks really awesome]. Problem is, [looks aren't everything], because there's just some things
    
    about this map that don't make much practical sense in a [tournament map].
    ...
    the railgun tower (from/to) that hallway would've made perfect sense.
    
    ==============================
    | Evil Playground/mptourney4 |
    ==============================
    
    This map is without a doubt, [extremely well made]. It isn't as small as [House of Decay], but it has a really
    well thought out approach to [multi-tiered combat], which is a signature of extremely well made maps. The only
    thing that I do not like about this map, is the fact that it has [death pits], and they're pretty annoying.
    
    However, that's just a pet peeve at this point, because I'm not going to tank its score based on that...
    The combat is [extremely fun], [challenging], and the [pendulum of fairness] can swing in either direction-
    which is an indication of an [extremely well made tournament map].
                                                                                                             __________/
...
/¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯                                                                                                  
    ...
    The [mapping process] takes a while to complete, and typically speaking, without a lot of experience, you won't
    be able to create a [really good map] in a single day. Even the map that I made in a single day in this particul
    ar 
    video... 
    ...
    \____________|_______________________|______________________|______________________________/
    
    ...isn't finished. Ever since I created that level, I've thought about changing things in it to make it more fun
    
    to play. As it stands, it has some issues that stem from [being incomplete]. I wouldn't expect it to get a (10/1
    0)
    like [mptourney1] or [mptourney4].
    
    However, there is an issue about that map which leads me to believe that no amount of changes will be able to ma
    ke
    it a [viable competitive level]. And, the issue that leads me to believe as such, stems from...
...
/¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯                                                                                                  
    ...
    A man by the name of [Carl Sagan], a famed [science communicator] that won a [Pulitzer prize] for his books
    [The Dragons of Eden], and for [Cosmos: A Personal Voyage] once raised an analogy called...
    ...
    Yeah, well, [John Carmack] and [John Romero] probably grew up playing [Atari] for years, watching [Carl Sagan]
    in this show [Cosmos: A Personal Voyage], until they eventually said to each other...
    ...
    And from that day forward, [(2) Johns] took an oath to embark on a journey to make a game that had [depth], as
    they went about changing the entire way that games were played by people all around the world.
    
    (It's worth noting that there were ALSO [(2) Carmacks]...)
    ...
    [Doom] raised the bar to such an immensely high level, that the game actually had [horizontal + vertical depth].
    
    
    But then, in (1996), with the release of a cool game called [Quake], they permanently impacted the world by
    making the first FPS that was truly [3D], and wasn't [flat]... (though [Descent] from (1995) officially holds
    that title, [Descent]'s gameplay was (fun, but) quite disorienting.)
    ...
    ...there had been a limitation to where the z-buffer could not allow multiple rooms to be stacked above or below
    
    one another without some sort of [hacky view portal magic].
    ...
    A [sphere] appears out of thin [flat] air as a [circle], and grows to a pretty large size.
    Not unlike how [id Software] grew into an industry giant...
    
    At which point, the [flatlanders] who saw the [sphere] pass through their dimension, they began to question
    their [flat] sanity, asking each other: 
...
/¯¯¯¯¯¯¯¯¯¯¯                                                                                                            
    ...
    ...well, a lot of [trial and error] is in order, before anyone has a great idea of how to do that, as well as
    how to arrange the [items] in that map.
    
    For instance, in [House of Decay/mptourney1], there's a lot of areas and choke points that allow each weapon
    to be better suited for the situation.
    ...
    //¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯\
    \
    \\            Weapon Mechanics : In [Quake III (Arena/*Team Arena)], there are a number of weapons             /
    /
     \\___________________________________________________________________________________________________________//
    
     /¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯\
    
     \  gauntlet | machinegun | shotgun | grenade launcher | rocket launcher | lightning gun | plasmagun | BFG10K  /
    
      \                                                                                                           /
       \                              nailgun | proximity mine launcher | chaingun                               /
    ...
    This is [extremely useful], in basically any scenario, but it does not do a
    lot of damage unless you are accurate and consistently hitting the target.
    ...
    [=] start with (10) grenades, (5) per [grenade] pickup
    ...
    [=] start with (10) rockets, (5) per [rocket] pickup
    ...
    This gun is really useful for mid range, though it can also be used effectively in short or 
    long range, depending on whether the user can sufficiently track the movement of a target.
    ...
    This gun is basically a rapid-fire version of the [rocket launcher].
    
    It carries many of the same characteristics, but its rounds move faster than [rockets].
    Typically, this would NOT be a good choice to include in a tournament level, as it makes the game quite unfair.
    ...
    /    End of [weapon mechanics], extended guide here: [https://strategywiki.org/wiki/Quake_III_Arena/Weapons]    
    \
    \_______________________________________________________________________________________________________________
    /
    ...
    //¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯\
    \
    \\               Item Mechanics : In [Quake III (Arena/Team Arena)], there are a number of items               /
    /
     \\___________________________________________________________________________________________________________//
     ...
     \                                                                                                             /
      ...
      \                                Health | Armor | Usable | Powerups | Runes                                 /
       \                                                                                                         /
        ¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯
    ...
    [+]            scout : similar to haste, cannot pick up armor (possessed armor is removed)
...
/¯¯¯¯¯¯¯¯¯¯¯¯                                                                                                           
    ...
    There are a number of strategies that can be used in [Quake III (Arena/Team Arena)] that aren't readily apparent
    ,
    and I will not go into great depth about them, because the geometry of any given map actually changes the strate
    gies
    that can be used, in a variety of ways.
    
    [Choke points] are pretty good to use, [bunny hopping + strafe jumping] are also good to use.
    [Rocket jumps] can be useful.
    
    [Plasma wall boosting] can also be useful... but there are a lot of other strategies which just won't be
    visually apparent, unless the [shape] of the map provides a distinct scenario.
    ...
    \____________|_________________|____________|______________________________/
    ...
    Using the [spawn location positioning strategy] is harder to pull off, because in certain positions, it is
    impossible to hear where [Fritzkrieg] is spawning.
    
    Generally speaking, there is a formula to it, but it is STILL rather random, and depends on where the player is
    on the map when they kill the bot, as well as where the bot is when they die... and I think it randomly selects
    that location, depending on where the player is at the time of respawn.
    ...
    First off, if it is true that [Death Factory] has a lot of [spawn points], the best thing to do...
    ..is to guard items.
    
    In all (4) of these levels, there is a consistent amount of effort going into [picking up health and armor].
    
    That is a [general strategy], and it is a core central focus to this game, just like collecting [minerals]
    and [vespene gas] is, in [Starcraft]. [Health] and [armor] are basically [resources] in this game.
    ...
    Using that as a known [fact], that allows me to collect the necessary weapons, and then guard certain areas...
    ...particularly the [rocket launcher] area.
    ...
    disadvantage to put myself in.
    
    One such disadvantage, is allowing [Pi] to collect the [railgun], because ALL bots on [nightmare] have an
    incredibly deadly level of accuracy with the [railgun].
    ...
    ...because she will occasionally spawn next to the [railgun], and there's only a [yellow armor] and some
    [armor shards] in the level, that give any player a way to protect themselves from up to (2) shots.
    ...
    Whereas in [House of Decay], there are definitely (3) tiers...  though, the [bottom tier] is limited to a
    pool of water, that's where the armor is.
    
    Nobody will WANT to go there, if it is not there, because of how [VULNERABLE] they will be, in that position.
...
/¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯                                                                                                    
    ...
    I may have mentioned numerous times that [multi-tiered] levels are pretty clutch, and essential to a map
    having a [replayability] factor to it. When taking a closer look at the tournament levels from [Quake III Arena]
    ...
    ...
    Otherwise, the level [LOOKS really nice], which goes a long way in receiving a mediocre score, but the general f
    low
    of the map feels very, very [flat]. That's because [the gameplay certainly is].
    ...
    This map was part of [Q3Test], and left a pretty deep impression on me long before the full game was released.
    This map [looks great], [plays great], and is pretty tough against [Hunter].
    ...
    It has a lot of vertical gameplay, and... I would encourage people to take inspiration from this map, 
    how it's [made], how it [flows], and how it [plays]... if you want to build a really great level.
    ...
    However, [Space Station 1138] is a pretty competitive space based tournament level with vertical gameplay...
    I don't think it ever received a title or designation to where people would play it.
...
/¯¯¯¯¯¯¯                                                                                                                
    ...
    The [shaping] process can actually take days, but the end result will be something highly polished, and this can
    
    be seen in a number of levels that were made for [Quake Live], like... [Heavy Rain].
    ...
    Then there's also [Blood Run], which is the most official tournament level I can think ofI that was never a part
    
    of the game itself. That too, has a [shape].
    
    But- even if we take a look at [Quake II], and even [Quake I],
    there are plenty of examples of great maps there, too.
    ...
    People still play [q2dm1] competitively, it's a great example of a map that works for tournament and free-for-al
    l,
    but every adaptation to [Quake III Arena] has been less than exhilirating than the original. 
    
    It has a lot of vertical gameplay to it, [multiple tiers], a pretty balanced layout, and it has a really unique
    shape to it that I've never seen be reproduced in a manner to where I was as impressed, or blown away.
    
    [q2dm1] is by far, [one of the best maps I've ever seen].
    And, even the rest of the maps I just listed, they're really good.
    
    Even [The Pits/q2dm5] is good... it's just that some of these maps really delve into being a bit of a breathing
    entity itself, with an environment.
    
    But NOW, the question is, how does one go about [creating the shape] for these maps...?
    
    Well, imagine if you would, that you could have a conversation with [Carl Sagan] himself, and ask him how to bui
    ld
    a 4-dimensional tesseract, and then build a level off of that.
    ...
    The bottom line, is that a good map has to have a [general flow] to it, where players can traverse the map in
    multiple ways, and further to that point, it has to be really polished, have a [good AMOUNT of flow to it], have
    
    the [items] placed in ways where it makes perfect sense, and is advantageous to the players... and then you have
    
    to top it all off with well executed aesthetics, geometry, and lighting.
    ...
    [Tokays Towers] is a great example of a level that has vertical gameplay down pat, and... to this day,
    I sometimes think about how I would go from one end of the map to the other, top to bottom, et cetera.
    
    The only thing I never liked about it is [swimming through the water].
    ...
    Because, a rough draft of a level doesn't even need to be textured or lit all that well, for a map to have some
    REALLY promising [gameplay] to it. The point being, creating the [structure] of the map AROUND the [item placement],
    and [item control]... is a really good idea.
                                                                                                                _______/
...
/¯¯¯¯¯¯¯¯¯¯¯¯                                                                                                           
    ...
    [To be continued...]
                                                                                                           ____________/
\_________________________________________________________________________________________________________/ Conclusion  
#>

$Doc = New-Document -Name "Game Design (Part I)" -Date 08/30/2023
...
To narrow the objective even further, we'll be focusing on [Tournament/1v1] mode, as well as how to make a really
(cool + fun) level for it, that focuses on aspects of the [mode] and the [game], in order to produce something
that is incredibly polished.
...
The [mapping process] takes a while to complete, and typically speaking, without a lot of experience, you won't
be able to create a [really good map] in a single day. Even the map that I made in a single day in this particular 
video... 
...
...isn't finished. Ever since I created that level, I've thought about changing things in it to make it more fun
to play. As it stands, it has some issues that stem from [being incomplete]. I wouldn't expect it to get a (10/10)
like [mptourney1] or [mptourney4].

However, there is an issue about that map which leads me to believe that no amount of changes will be able to make
it a [viable competitive level]. And, the issue that leads me to believe as such, stems from...
...
$Doc.Add("Strategies",@"
There are a number of strategies that can be used in [Quake III (Arena/Team Arena)] that aren't readily apparent,
and I will not go into great depth about them, because the geometry of any given map actually changes the strategies
that can be used, in a variety of ways.
...
Otherwise, the level [LOOKS really nice], which goes a long way in receiving a mediocre score, but the general flow
of the map feels very, very [flat]. That's because [the gameplay certainly is].
...
People still play [q2dm1] competitively, it's a great example of a map that works for tournament and free-for-all,
but every adaptation to [Quake III Arena] has been less than exhilirating than the original. 
...
Well, imagine if you would, that you could have a conversation with [Carl Sagan] himself, and ask him how to build
a 4-dimensional tesseract, and then build a level off of that.
