    ...
    Stage([String]$Name)
    {
        ...
        Switch ($Name)
        {
            ViewUid
            {
                ...
                $Ctrl.Xaml.IO.ViewUidOutput.Add_DoubleClick(
                {
            ...
            EditUid
            {
                ...
                <#
                $Ctrl.Xaml.IO.EditUidOutput                = 
                $Ctrl.Xaml.IO.EditUidRecordRefresh         = 
                $Ctrl.Xaml.IO.EditUidRecord                = 
                #>
            }
            ViewClient
            {<#
...
<# 
    What I want to do, is call all of the XAML types and then layer them within commenting code,
    so that I know what things to slowly work away at for each individual section, in reference
    to 1) initial, 2) stage, and 3) handle

    The Initial will be the initial values for items in the GUI.
    Like I mentioned, this can be done through the XAML.
    But, PowerShell can handle that as well.

    Stage will strictly be meant for the event handlers, and things of that nature.

    Handle will be a switchable mode for basically whether or not a given
    XAML property/control is of a certain value.

    So for instance, if I make changes to the text field in any given field
    that has a label that says "Search", then editing the text field in that
    text box will actually raise the event handler, which will in turn be handled
    by the handle method and it will have a switch within the handle method that
    dictates whether the items are reverted to a default value, or something different.

    This can be used to enable/disable portions of the GUI.    
#>

<# 
    Script to extract everything in the order I'd like the controls to be in
$List = @( )
ForEach ($X in 0..($Ctrl.Database.Mode.Output.Count-1))
{
    $Item  = $Ctrl.Database.Mode.Output[$X]
    $List += "# [{0}]" -f $Item.Name
    $Ctrl.Xaml.Types | ? Name -match $Item.Name | % {

        $List += '$Ctrl.Xaml.IO.{0}' -f $_.Name
    }
    $List += ""
}
...
$Max = @{ 
    ...
    Length = ($List | Sort-Object Length)[-1].Length
}
...
ForEach ($Line in $List)
{
    If ($Line -match "^\`$Ctrl")
    {
        $Out += "{0} = " -f $Line.PadRight($Max.Length," ")
    }
    Else
    {
        $Out += $Line
