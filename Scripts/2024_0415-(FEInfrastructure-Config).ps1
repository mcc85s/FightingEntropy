# 04/15/2024 - Retooling all of the config classes + controller classes, simplifying the underlying code behind
# AND updating the utlity seen in this video: [FightingEntropy(π)][FEInfrastructure]: https://youtu.be/6yQr06_rA4I
...
Class AddsServerObject
{
    [String]              $Name
    [String]             $Class
    [String]              $Guid
    [String] $DistinguishedName
    AddsServerObject([Object]$Object)
    {
    ...
    [String] ToString()
    {
        Return "<FEInfrastructure.Config.AddsServer.Object>"
    }
}

Class AddsServerObjectList
{
    [String]   $Name
    [Object] $Output
    AddsServerObjectList([String]$Name)
    {
        $This.Name = $Name
    }
    Clear()
    ...
    Refresh([Object[]]$Object)
    {
        ...
        ForEach ($Item in $Object)
        {
            $This.Output += $This.AddsServerObject($Item)
        }
    }
    [Object] AddsServerObject([Object]$Object)
    {
        Return [AddsServerObject]::New($Object)
    }
    [String] ToString()
    {
        Return "<FEInfrastructure.Config.AddsServer.Object.List>"
    }
}

Class AddsServerDomain
{
    [String]   $HostName
    [String]     $DCMode
    [String] $DomainMode
    ...
    [Object] AddsServerObjectList([String]$Name)
    {
        Return [AddsServerObjectList]::New($Name)
    }
    GetLists()
    {
        $This.Object      = @( )

        ForEach ($Name in [System.Enum]::GetNames([AddsServerObjectType]))
        {
            $This.Object += $This.AddsServerObjectList($Name)
        }
    }
    [Object] GetList([String]$Name)
    {
        Return $This.Object | ? Name -eq $Name
    }
    Refresh()
    {
        $Hash            = @{ }

        ForEach ($Item in $This.GetAdObject($This.Config,"(^Site$|^Sitelink$|^Subnet$|^Dhcpclass$)"))
        {
            $Hash.Add($Hash.Count,$Item)
        }

        ForEach ($Item in $This.GetAdObject($This.Root,"(^OrganizationalUnit$|^Computer$)"))
        {
            $Hash.Add($Hash.Count,$Item)
        }

        $ObjectList      = $Hash[0..($Hash.Count-1)]

        # Site
        $xSite           = $ObjectList  | ? ObjectClass -eq Site
        $This.GetList("Site").Refresh($xSite)

        # Sitelink
        $xSiteLink       = $ObjectList  | ? ObjectClass -eq Sitelink
        $This.GetList("SiteLink").Refresh($xSiteLink)

        # Subnet
        $xSubnet         = $ObjectList | ? ObjectClass -eq Subnet
        $This.GetList("Subnet").Refresh($xSubnet)

        # Dhcp
        $xDhcp           = $ObjectList  | ? ObjectClass -eq DhcpClass
        $This.GetList("DhcpClass").Refresh($xDhcp)

        # Organizational Unit
        $xOU             = $ObjectList | ? ObjectClass -eq OrganizationalUnit
        $This.GetList("OrganizationalUnit").Refresh($xOU)

        # Computer
        $xComputer       = $ObjectList | ? ObjectClass -eq Computer
        $This.GetList("Computer").Refresh($xComputer)
    }
    [String] ToString()
    {
...
### Adds Server Site stuff
Class AddsServerSiteReference
{
    [UInt32]             $Index
    [String]              $Name
    [String]          $SiteName
    ...
    AddsServerSiteReference([UInt32]$Index,[Object]$Site,[String]$Base)
    {
        $This.Index             = $Index
    ...
    [String] ToString()
    {
        Return "<FEInfrastructure.Config.AddsServer.Site.Reference>"
    }
}

Class AddsServerSiteState
{
    ...
    {
        Return @([AddsServerSiteState]::List | % GetEnumerator | ? Value -match $Code | % Name)
    }
    Static [String] GetCode([String]$Name)
    {
        Return @([AddsServerSiteState]::List | % GetEnumerator | ? Name -eq $Name | % Value)
    }
    AddsServerSiteState()
    {
    ...
    [String] ToString()
    {
        Return "<FEInfrastructure.Config.AddsServer.Site.State>"
    }
}

Class AddsServerSiteZipEntry
{
    [String]     $Zip
    [String]    $Type
    [String]    $Name
    ...
    [String]    $Long
    [String]     $Lat
    AddsServerSiteZipEntry([Object]$Object)
    {
        If ($Object -match "^\d{5}$")
        {
            $This.Zip       = $Object
    ...
    [String] ToString()
    {
        Return "<FEInfrastructure.Config.AddsServer.Site.ZipEntry>"
    }
}

Class AddsServerSiteZipMaster
{
    [String]    $Path
    [Object] $Content
    [Object]  $Output
    AddsServerSiteZipMaster([String]$Path)
    {
        $This.Path    = $Path
    ...
    [Object] AddsServerSiteZipEntry([String]$Line)
    {
        Return [AddsServerSiteZipEntry]::New($Line)
    }
    [Object] AddsServerSiteZipEntry([UInt32]$Zip)
    {
        Return [AddsServerSiteZipEntry]::New($Zip)
    }
    [Object] Zip([Object]$Zip)
    {
        ...
        If (!$Index)
        {
            Return $This.AddsServerSiteZipEntry($Zip)
        }
        Else
        {
            Return $This.AddsServerSiteZipEntry($This.Content[$Index])
        }
    }
    [String] ToString()
    {
        Return "<FEInfrastructure.Config.AddsServer.Site.ZipMaster>"
    }
}

Class AddsServerSite
{
    [String]  $Organization
    [String]    $CommonName
    Hidden [String]   $Type
    ...
    AddsServerSite([String]$Organization,[String]$CommonName,[Object]$Zip)
    {
        $This.Organization     = $Organization
        $This.CommonName       = $CommonName
        $This.Type             = $Zip.Type
    ...
    {
        Return [AddsServerSiteState]::GetName($Code)
    }
    [Object] GetStateCodeByName([String]$Name)
    {
        Return [AddsServerSiteState]::GetCode($Name)
    }
    GetSiteLink()
    {
        $Return                = @{ }
    ...
    [String] ToString()
    {
        Return "<FEInfrastructure.Config.AddsServer.Site>"
    }
}

Class AddsServerSiteMaster
{
    [Object]    $Output
    [Object] $Reference
    AddsServerSiteMaster()
    {
        $This.Clear()
    }
    Clear()
    ...
    [String] ToString()
    {
        Return "<FEInfrastructure.Config.AddsServer.Site.Master>"
    }
}

### AddsServerSubnet stuff
Class AddsServerSubnetReference
{
    [UInt32]             $Index
    [String]              $Name
    [Object]           $Network
    [UInt32]            $Exists
    [Object] $DistinguishedName
    AddsServerSubnetReference([UInt32]$Index,[Object]$Subnet,[String]$SearchBase)
    {
        $This.Index             = $Index
    ...
    [String] ToString()
    {
        Return "<FEInfrastructure.Config.AddsServer.Subnet.Reference>"
    }
}

Class AddsServerSubnet
{
    [String]            $Name
    [String]         $Network
    [UInt32]          $Prefix
    ...
    AddsServerSubnet([String]$Network)
    {
        If ($Network -notmatch "(\d+\.){3}\d+\/\d+")
        {
        ...
        $This.Network   = $X[0]
        $This.Prefix    = $X[1]
        $This.Netmask   = $This.GetSubnetMask()
        $This.HostCount = $This.GetHostCount()
    ...
    {
        $Binary = @(0..31 | % { [Int]($_ -lt $This.Prefix) }) -join ""
        Return @(0,8,16,24 | % { [Convert]::ToInt32($Binary.Substring($_,8),2) }) -join "."
    }
    [UInt32] GetHostCount()
    {
        $Count = @($This.SubnetMask -Split "\." | % { @(256-$_;256)[$_ -eq 0] }) -join "*" | Invoke-Expression
        Return $Count - 2
    }
    GetHostRange()
    {
        $xNetwork       = [UInt32[]]($This.Network -Split "\.")
        $xNetmask       = [UInt32[]]($This.Netmask -Split "\.")

        $xHostRange     = @( )
        ForEach ($X in 0..3)
        {
            $xHostRange += Switch ($xNetmask[$X])
            {
                0       { "0..255" }
                255     { $xNetwork[$X] }
                Default { "{0}..{1}" -f $xNetwork[$X],($xNetwork[$X]+((256-$xNetmask[$X])-1)) }
            }
        }

        $This.HostRange = $xHostRange -join "/"
        ...
        ForEach ($X in 0..3)
        {
            ...
            {
                0       { $Hosts.Add($X,@(0,255)) }
                255     { $Hosts.Add($X,$xNetwork[$X])  }
                Default { $Hosts.Add($X,@($xNetwork[$X],($xNetwork[$X] + ((256 - $xNetMask[$X]) - 1)))) }
            }
        }
        ...
        ForEach ($0 in $Hosts[0])
        {
            ForEach ($1 in $Hosts[1])
            {
                ForEach ($2 in $Hosts[2])
                {
                    ...
                    {
                        $Addresses.Add($Addresses.Count,"$0.$1.$2.$3")
                    }
                }
            }
        }

        $This.Broadcast = $Addresses[$Addresses.Count-1]

        $X              = [UInt32[]]($This.Network -Split "\.")
        $X[-1]          = $X[-1] + 1
        $This.Start     = $X -join "."

        $Y              = [UInt32[]]($This.Broadcast -Split "\.")
        $Y[-1]          = $Y[-1] - 1
        $This.End       = $Y -join "."
    }
    GetReverseDNS()
    {
        $xNetmask       = $This.Netmask -Split "\." | % { 256 - $_ }
        $xNetwork       = $This.Network -Split "\."

        $Reverse        = ForEach ($X in 3..0)
        {
            If ($xNetmask[$X] -ne 256)
            {
                $xNetwork[$X]
            }
        }

        $This.ReverseDNS = "{0}.in-addr.arpa" -f ($Reverse -join ".")
    }
    [String] ToString()
    {
        Return "<FEInfrastructure.Config.AddsServer.Subnet>"
    }
}

Class AddsServerSubnetMaster
{
    [Object]    $Output
    [Object] $Reference
    AddsServerSubnetMaster()
    {
        $This.Clear()
    }
    Clear()
    ...
    [String] ToString()
    {
        Return "<FEInfrastructure.Config.AddsServer.Subnet.Master>"
    }
}

# SiteLink stuff
Class AddsServerSiteLinkReference
{
    [UInt32]             $Index
    [String]              $Name
    ...
    [UInt32]            $Exists
    [Object] $DistinguishedName
    AddsServerSiteLinkReference([UInt32]$Index,[Object]$SiteLink,[String]$SearchBase)
    {
        $This.Index             = $Index
        $This.Name              = $SiteLink.Name
        $This.DistinguishedName = "CN=$($This.Name),CN=Sites,$SearchBase"
    }
    [String] ToString()
    {
        Return "<FEInfrastructure.Config.AddsServer.SiteLink.Reference>"
    }
}

Class AddsServerSiteLink
{
    [String]      $Organization
    [String]        $CommonName
    [String]              $Name
    ...
    [String] $DistinguishedName
    [UInt32]            $Exists
    AddsServerSiteLink([Object]$Site,[Object]$Subnet)
    {
        ...
        $This.Start             = $Subnet.Start
        $This.End               = $Subnet.End
        $This.Range             = $Subnet.Range
        $This.Broadcast         = $Subnet.Broadcast
        $This.ReverseDNS        = $Subnet.ReverseDNS
    }
    [String] ToString()
    {
        Return "<FEInfrastructure.Config.AddsServer.SiteLink>"
    }
}

Class AddsServerSiteLinkMaster
{
    [Object]    $Output
    [Object] $Reference
    AddsServerSiteLinkMaster()
    {
        $This.Clear()
    }
    Clear()
    ...
    [String] ToString()
    {
        Return "<FEInfrastructure.Config.AddsServer.SiteLink.Master>"
    }
}

Class AddsServerComputer
{
    [String]      $Organization
    [String]        $CommonName
    [String]              $Site
    ...
    [Object]          $Computer
    [String]              $Guid
    AddsServerComputer([Object]$Site,[Object]$Node)
    {
        $This.Organization      = $Site.Organization
        $This.CommonName        = $Site.CommonName
    ...
    [String] ToString()
    {
        Return "<FEInfrastructure.Config.AddsServer.Computer>"
    }
}

Class AddsServerComputerMaster
{
    [Object]    $Output
    [Object] $Reference
    AddsServerComputerMaster()
    {
        $This.Clear()
    }
    Clear()
    ...
    [String] ToString()
    {
        Return "<FEInfrastructure.Config.AddsServer.Computer.Master>"
    }
}

Class AddsServerUser
{
    [String]      $Organization
    [String]        $CommonName
    [String]              $Site
    ...
    [String]              $Guid
    AddsServerUser([Object]$Site,[Object]$Node)
    {
        $This.Organization      = $Site.Organization
        $This.CommonName        = $Site.CommonName
        $This.Site              = $Site.SiteLink
    ...
    [String] ToString()
    {
        Return "<FEInfrastructure.Config.AddsServer.User>"
    }
}

Class AddsServerUserMaster
{
    [Object]    $Output
    [Object] $Reference
    AddsServerUserMaster()
    {
        $This.Clear()
    }
    Clear()
    ...
    [String] ToString()
    {
        Return "<FEInfrastructure.Config.AddsServer.User.Master>"
    }
}

# AddsServer Controller stuff
...
Class AddsServer
{
    ...
    [Object]     $Computer
    [Object]         $User
    AddsServer([Object]$Module)
    {
        $This.Domain       = $This.AddsServerDomain()
        $This.Postal       = $This.AddsServerSiteZipMaster($Module._Control("zipcode.txt").Fullname)
        $This.Site         = $This.AddsServerSiteMaster()
        $This.Subnet       = $This.AddsServerSubnetMaster()
        $This.SiteLink     = $This.AddsServerSiteLinkMaster()
        $This.Computer     = $This.AddsServerComputerMaster()
        $This.User         = $This.AddsServerUserMaster()
    }
    [Object] AddsServerDomain()
    {
        Return [AddsServerDomain]::New()
    }
    [Object] AddsServerSiteMaster()
    {
        Return [AddsServerSiteMaster]::New()
    }
    [Object] AddsServerSiteZipMaster([String]$Path)
    {
        Return [AddsServerSiteZipMaster]::New($Path)
    }
    [Object] AddsServerSite([UInt32]$ZipCode)
    {
        Return [AddsServerSite]::New($This.Organization,$This.CommonName,$This.Postal.Zip($ZipCode))
    }
    [Object] AddsServerSiteReference([UInt32]$Index,[Object]$Site,[String]$SearchBase)
    {
        Return [AddsServerSiteReference]::New($Index,$Site,$SearchBase)
    }
    [Object] AddsServerSubnetMaster()
    {
        Return [AddsServerSubnetMaster]::New()
    }
    [Object] AddsServerSubnet([String]$Subnet)
    {
        Return [AddsServerSubnet]::New($Subnet)
    }
    [Object] AddsServerSubnetReference([UInt32]$Index,[Object]$Subnet,[String]$SearchBase)
    {
        Return [AddsServerSubnetReference]::New($Index,$Subnet,$SearchBase)
    }
    [Object] AddsServerSiteLinkMaster()
    {
        Return [AddsServerSiteLinkMaster]::New()
    }
    [Object] AddsServerSiteLinkReference([UInt32]$Index,[Object]$SiteLink,[String]$SearchBase)
    {
        Return [AddsServerSiteLinkReference]::New($Index,$SiteLink,$SearchBase)
    }
    [Object] AddsServerSiteLink([Object]$Site,[Object]$Subnet)
    {
        Return [AddsServerSiteLink]::New($Site,$Subnet)
    }
    [Object] AddsServerSiteLinkMaster()
    {
        Return [AddsServerSiteLinkMaster]::New()
    }
    [Object] AddsServerComputerMaster()
    {
        Return [AddsServerComputerMaster]::New()
    }
    [Object] AddsServerUserMaster()
    {
        Return [AddsServerUserMaster]::New()
    }
    ...
    [String] SiteSearchBase()
    {
        Return "CN=Sites,CN=Configuration,{0}" -f $This.DomainDn()
    }
    [String] SubnetSearchBase()
    {
        Return "CN=Configuration,{0}" -f $This.DomainDn()
    }
    [String] SiteLinkSearchBase()
    {
        Return "CN=Inter-Site Transports,CN=Sites,CN=Configuration,{0}" -f $This.DomainDn()
    }
    ...
    [String] UserPrincipalNameString([Object]$User)
    {
        Return ("{0}@{1}" -f $User.Name, $This.CommonName)
    }
    [String] LocationString([Object]$Site)
    {
        If ($Site.GetType -match "AddsServerSite")
        {
            Return ("{0}, {1} {2} [{3}]" -f $Site.Location, $Site.Region, $Site.Postal, $Site.Sitelink)
        }
        Else
        {
            Return $Null
    ...
    CreateSiteTemplate([UInt32]$Postal)
    {
        If (!$This.Organization -or !$This.CommonName)
        {
            Throw '[!] Must set the domain via $This.SetDomain($Org,$CN)'
        }
        ElseIf ($Postal -in $This.Site.Output.Postal)
        {
            Throw '[!] Location is already designated'
        }
        Else
        {
            $This.Site.Output    += $This.AddsServerSite($Postal)
        }
    }
    RemoveSiteTemplate([UInt32]$Postal)
    {
        If ($Postal -notin $This.Site.Output.Postal)
        {
            Throw '[!] Invalid location designated'
        }
        Else
        {
            $This.Site.Output     = @($This.Site.Output | ? Postal -ne $Postal)
        }
    }
    GetSite()
    {
        ...
        $List                     = $This.Domain.GetList("Site").Output

        ForEach ($Site in $This.Site.Output)
        {
            $Item                 = $This.AddsServerSiteReference($This.Site.Output.Count,$Site,$This.SiteSearchBase())
            $Item.Exists          = @(0,1)[$Item.DistinguishedName -in $List.DistinguishedName]
            $Site.SiteDN          = $Item
            $This.Site.Reference += $Item
        }
    }
    CreateSite()
    {
        ForEach ($Site in $This.Site.Reference)
        {
            $Location = $This.Site.Output | ? Sitelink -match $Site.Name | % { "{0}, {1} {2}" -f $_.Location, $_.Region, $_.Postal }
    ...
    CreateSubnetTemplate([String]$Subnet)
    {
        If ($Subnet -notmatch "(\d+\.){3}\d+\/\d+")
        {
            Throw "[!] Invalid entry, must provide this format: 'IpAddress/PrefixLength'"
        }
        ElseIf ($Subnet -in $This.Subnet.Output.Name)
        {
            Throw '[!] Subnet is already designated'
        }
        Else
        {
            $This.Subnet.Output    += $This.AddsServerSubnet($Subnet)
        }
    }
    RemoveSubnetTemplate([String]$Subnet)
    {
        If ($Subnet -notin $This.Subnet.Output.Name)
        {
            Throw '[!] Invalid subnet designated'
        }
        Else
        {
            $This.Site.Output     = @($This.Site.Output | ? Name -ne $Subnet)
        }
    }
    GetSubnet()
    {
        $This.Subnet.Reference      = @( )
        $This.Domain.Refresh()

        $List                       = $This.Domain.GetList("Subnet").Output

        ForEach ($Network in $This.Subnet.Output)
        {
            $Item = $This.AddsServerSubnetReference($This.Subnet.Output.Count,$Network,$This.SubnetSearchBase())

            $Item.Exists            = @(0,1)[$Item.DistinguishedName -in $List.DistinguishedName]
            $Network.SubnetDN       = $Item
            
            $This.Subnet.Reference += $Item
        }
    }
    CreateSubnet()
    {
        ForEach ($Network in $This.Subnet.Reference)
        {
            Switch ($Network.Exists)
            {
                ...
                1
                {
                    [Console]::WriteLine("Item [+] Exists [$($Network.DistinguishedName)]")
                }
            }
        }
    ...
    RemoveSubnet()
    {
        ForEach ($Network in $This.Subnet.Reference)
        {
            Switch ($Network.Exists)
            {
                0
                {
                    [Console]::WriteLine("Item [!] Does not exist [$($Network.DistinguishedName)]")
                }
                1
                {
                    Remove-ADObject -Identity $Network.DistinguishedName -Verbose -Confirm:0
                    $Network.Exists = 0
                }
            }
        }
    ...
    CreateSiteLinkTemplate([Object]$Site,[Object]$Subnet)
    {
        If ($Site.SiteLink -in $This.SiteLink.Output.SiteLink)
        {
            Throw "[!] Invalid entry, SiteLink already created"
        }
        Else
        {
            $This.SiteLink.Output    += $This.AddsServerSiteLink($Site,$Subnet)
        }
    }
    RemoveSiteLinkTemplate([String]$SiteLink)
    {
        If ($SiteLink -notin $This.SiteLink.Output.Name)
        {
            Throw '[!] Invalid SiteLink designated'
        }
        Else
        {
            $This.SiteLink.Output     = @($This.SiteLink.Output | ? Name -ne $SiteLink)
        }
    }
    GetSiteLink()
    {
        $This.SiteLink.Reference    = @( )
        $This.Domain.Refresh()

        $List                       = $This.Domain.GetList("SiteLink").Output

        ForEach ($SiteLink in $This.SiteLink.Output)
        {
            $Item = $This.AddsServerSiteLinkReference($This.SiteLink.Output.Count,$SiteLink,$This.SiteLinkSearchBase())

            $Item.Exists            = @(0,1)[$Item.DistinguishedName -in $List.DistinguishedName]
            
            $This.SiteLink.Reference += $Item
        }
    }
    CreateSiteLink()
    {
    ...
    CreateComputerTemplate()
    {

    }
    RemoveComputerTemplate()
    {
    ...
    GetComputer()
    {
        ...
        $List                       = $This.Domain.GetList("Computer").Output

        ForEach ($Computer in $This.Computer.Output)
        {
            $Item = $This.AddsServerComputerReference($This.Computer.Output.Count,$Computer,$This.ComputerSearchBase())

            $Item.Exists            = @(0,1)[$Item.DistinguishedName -in $List.DistinguishedName]
            
            $This.Computer.Reference += $Item
        }
    }
    CreateComputer()
    {
    ...
    GetUser()
    {
        ...
        $List                       = $This.Domain.GetList("User").Output

        ForEach ($User in $This.User.Output)
        {
            $Item = $This.AddsServerUserReference($This.User.Output.Count,$User,$This.UserSearchBase())

            $Item.Exists            = @(0,1)[$Item.DistinguishedName -in $List.DistinguishedName]
            
            $This.User.Reference += $Item
        }
    }
    CreateUser()
    {
    ...
    [String] ToString()
    {
        Return "<FEInfrastructure.Config.AddsServer>"
    }
}

$Module = Get-FEModule -Mode 1
$Adds   = [AddsServer]::New($Module)

$Adds.SetDomain($Module.Company,"securedigitsplus.com")

# Site template
$Adds.CreateSiteTemplate(12065)
$Adds.GetSite()
$Adds.CreateSite()

# Subnet template
$Adds.CreateSubnetTemplate("192.168.42.0/24")
$Adds.GetSubnet()
$Adds.CreateSubnet()


<# Most of the stuff below will probably be eliminated in entirety
