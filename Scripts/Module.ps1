    ...
    Class FileProperty
    {
        ...
        {
            $This.Url      = "{0}\blob\main\{1}\{2}?raw=true" -f $Base, $This.Folder(), $This.Name
        }
        AssignContent()
        {
            Try
    ...
    Class ModuleProperty
    {
        [String] $Name
        [String] $Fullname
        ModuleProperty([String]$Name,[String]$Fullname)
        {
            $This.Name     = $Name
    ...
    Class ModuleBase
    {
        ...
        [Object]     $File
        [Object] $Manifest
        ModuleBase([String]$Version,[String]$Resource,[String]$Path)
        {
            $This.Registry = $This.Set(0,"HKLM:\Software\Policies\Secure Digits Plus LLC\FightingEntropy\$Version")
            $This.Resource = $This.Set(1,"$Resource")
            $This.Root     = $This.Set(2,"$Path")
            $This.Main     = $This.Set(3,"$Path\FightingEntropy")
            $This.Trunk    = $This.Set(4,"$Path\FightingEntropy\$Version")
            $This.File     = $This.Set(5,"$Path\FightingEntropy\$Version\FightingEntropy.psm1")
            $This.Manifest = $This.Set(6,"$Path\FightingEntropy\$Version\FightingEntropy.psd1")
        }
        [String] Slot([UInt32]$Type)
        {
            Return @("Registry","Resource","Root","Main","Trunk","File","Manifest")[$Type]
        }
        [Object] Set([UInt32]$Type,[String]$Path)
        {
            $This.TestPath($Path)

            Return [ModuleProperty]::New($This.Slot($Type),$Path)
        }
    ...
    Class ModuleMain
    {
        ...
        [Object] GetBase()
        {
            ...
            {
                ^Win32_ { $Env:PSModulePath -Split ";" -Match "Program Files" }
                Default { $Env:PSModulePath -Split ":" -Match "PowerShell"    }
            }

            Return [ModuleBase]::New($This.Version,$Resource,$Path)
        }
        [Object] GetManifest()
        {
            Return [FileManifest]::New()
        }
    }

    $Module = [ModuleMain]::New()
