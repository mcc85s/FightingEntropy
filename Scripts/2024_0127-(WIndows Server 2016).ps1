...
# // Transmit scriptlet
$Activity = "Install: KB4486129"
$Vm.Update(0,"[~] Transmitting (Scriptlet) : $Activity")
...
Start-TcpSession -Client -Source $Source -Port $Port -Content $Scriptlet | % Initialize

$Vm.TypeLine('{0}.Script += $Script.Content.Message -join ""')
$Vm.TypeKey(13)

$Vm.Update(1,"[+] Complete (Script) : []")
...
Start-TcpSession -Client -Source $Source -Port $Port -Content $Json | % Initialize

$Vm.TypeLine('$Object = $Script.Content.Message -join "" | ConvertFrom-Json')
$Vm.TypeKey(13)
$Vm.TypeLine('{0}.Script.Add($Object)')
$Vm.TypeKey(13)

$Vm.Update(1,"[+] Complete (Script) : [$Activity]")
...
ForEach ($Script in $Vm.Script.Output | ? Complete -eq 0)
{
    ...
    Switch ($Script.Name)
    {
        ...
        InstallChoco
        {
            $Script = $Vm.Script.Get("InstallChoco")
            $Scriptlet = @( 
            '$Script = {';
            'param({0})' -f $V
            '$Start = [DateTime]::Now';
        ...
        InstallVsCode
        {
            ...
            '$Start = [DateTime]::Now';
            $Script.Content.Line[0,1];
            '$Test = Get-Package | ? Name -match "Visual Studio Code"'
            '$Status = Switch ([UInt32]!!$Test)';
            '{';
...
'$Test = Get-Package | ? Name -match "^Microsoft Edge$"'
'$Status = Switch ([UInt32]!!$Test)';
'{';
'    0 { "[!] Microsoft Edge NOT installed" }';
'    1 { "[+] Microsoft Edge ({0}) installed" -f $Test.Version }';
'}';
'$Elapsed = [TimeSpan]([DateTime]::Now-$Start)';
'{0}.Status("$Status [$Elapsed]")' -f $V;
'}';
...
$Vm.Update(0,"[~] Install the Michaelsoft Deployment Toolkit + WinADK + WinPE")
$Scriptlet = @(
'$Script = {'
'param({0})' -f $V;
'$Start = [DateTime]::Now'
...
$Vm.Update(0,"[+] Set Group Policy on domain controller")
<# [ ] Set Group Policy
  - use powershell to make a group policy
  New-GPLink
  New-GPO
  New-GPStarterGPO
         ...
         > Local Policies
          > Security Options
           > Domain controller
             ...
             Enabled
             Vulnerable
    > Firewall entries if necessary via default action
      
# [X] Connect to custom account using RDP on newly made account
# [X] Connect to custom account using WinRM
...
$Splat.ComputerName = "server.securedigitsplus.com"
Enter-PSSession @Splat
