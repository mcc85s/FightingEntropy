...
{#>
    ...
    Class VmControllerXaml
    {
        ...
        '                             Content="[Switch]:"/>',
        '                    <TextBox Grid.Column="1"',
        '                             Text="(Manage/create/remove) virtual switches"',
        '                             IsReadOnly="True"/>',
        ...
        '                             Name="NetworkRefresh"/>',
        '                </Grid>',
        '                <DataGrid Grid.Row="3" Name="NetworkOutput">',
        '                    <DataGrid.Columns>',
        ...
        '                            <DataGridTemplateColumn.CellTemplate>',
        '                                <DataTemplate>',
        '                                    <CheckBox IsChecked="{Binding Profile, UpdateSourceTrigger=PropertyChanged}"/>',
        '                                </DataTemplate>',
        '                            </DataGridTemplateColumn.CellTemplate>',
        '                        </DataGridTemplateColumn>',
        '                    </DataGrid.Columns>',
        ...
        '                        <ColumnDefinition Width="45"/>',
        '                    </Grid.ColumnDefinitions>',
        '                    <ComboBox Grid.Column="0" Name="NetworkSwitchType" SelectedIndex="0">',
        '                        <ComboBoxItem Content="External"/>',
        '                        <ComboBoxItem Content="Internal"/>',
        '                        <ComboBoxItem Content="Private"/>',
        '                    </ComboBox>',
        '                    <TextBox  Grid.Column="1" Name="NetworkSwitchName"/>',
        '                    <Image    Grid.Column="2" Name="NetworkSwitchIcon"/>',
        '                    <ComboBox Grid.Column="3" Name="NetworkSwitchAdapter"/>',
        ...
        '                    <TextBox Grid.Column="1"',
        '                             Text="Create (credential objects/accounts)"',
        '                             IsReadOnly="False"/>',
        '                </Grid>',
        ...
        '                    <TextBox Grid.Column="1"',
        '                             Text="Load images for templates to utilize"/>',
        '                </Grid>',
        ...
        '                    </DataGrid.Columns>',
        '                </DataGrid>',
        '                <Button Grid.Row="5" Name="ImageAssign" Content="Assign"/>',
        '            </Grid>',
        '        </TabItem>',
        ...
        '                           Content="[Node]:"/>',
        '                    <TextBox Grid.Column="1"',
        '                             Text="Manages hosts + templates"',
        '                             IsReadOnly="True"/>',
        '                    <Button Grid.Column="2"',
        '                            Content="Refresh"',
        '                            Name="NodeHostRefresh"/>',
        '                </Grid>',
        '                <DataGrid Grid.Row="1"',
        '                          Name="NodeHost">',
        '                    <DataGrid.Columns>',
        ...
        '                </Grid>',
        '                <Border Grid.Row="3" Background="Black" Margin="4"/>',
        '                <DataGrid Grid.Row="4" Name="NodeHostExtension">',
        '                    <DataGrid.Columns>',
        '                        <DataGridTextColumn Header="Name"',
        '                                                    Binding="{Binding Name}"',
        '                                                    Width="150"/>',
    ...
    Class VmNetworkInterfaceItem
    {
        ...
        AddRange([UInt32]$Count,[String]$Notation)
        {
        ...
        {
            $Address = $Null
            $List    = $This.Host | ? Type -eq Host | ? Status -eq 0
            If ($List.Count -gt 0)
            {
    ...
    Class VmNetworkController
    {
        ...
        Refresh()
        {
            ...
            {
                $Item         = $This.VmNetworkConfigEntry([Switch]$False,$This.Output.Count,$VmSwitch)
    ...
    # Slowly going to implement stuff to this class as I scale down to the main controller class
    Class VmControllerMaster
    {
        ...
        [Object] Get([String]$Name)
        {
            ...
            Switch ($Name)
            {
                ...
                Network
                {
                    $This.Update(0,"Getting [~] VmNetworkController")
                    $Item = [VmNetworkController]::New()
                }
                Credential
                {
                    $This.Update(0,"Getting [~] VmCredential")
                    $Item = [VmCredentialMaster]::New()
                }
                Image
                {
		...
		CheckSwitchName()
        {
            ...
            $xFlag.Status = [UInt32]($Item.Text -in $This.Network.Switch.Output.Name)
        ...
        NetworkPanel()
        {
            ...
            $Ctrl.Xaml.IO.NetworkRefresh.Add_Click(
            {
                ...
                $Ctrl.SwitchConfig()
                $Ctrl.Reset($Ctrl.Xaml.IO.NetworkSwitchAdapter,$Ctrl.Network.Physical().Name)
            })
            ...
            $Ctrl.Xaml.IO.NetworkOutput.Add_SelectionChanged(
            {
                ...
                If ($Index -gt -1)
                {
                    ...
                    $Ctrl.Reset($Ctrl.Xaml.IO.NetworkAdapterOutput,$Ctrl.Property($Item.Adapter))
                    $Ctrl.Reset($Ctrl.Xaml.IO.NetworkConfigOutput,$Ctrl.Property($Item.Config))
                    $Ctrl.Reset($Ctrl.Xaml.IO.NetworkSwitchOutput,$Ctrl.Property($Item.Switch))
                    If ($Item.Base)
                    {
                        $Ctrl.Reset($Ctrl.Xaml.IO.NetworkBaseOutput,$Ctrl.Property($Item.Base))
                    }
                    If ($Item.Range)
                    {
                        $Ctrl.Reset($Ctrl.Xaml.IO.NetworkRangeOutput,$Item.Range)
                    }
                    If ($Item.Host)
                    {
                        $Ctrl.Reset($Ctrl.Xaml.IO.NetworkHostOutput,$Item.Host)
                    }
                    If ($Item.Dhcp)
                    {
                        $Ctrl.Reset($Ctrl.Xaml.IO.NetworkDhcpOutput,$Item.Dhcp)
                    }
                }
            })
            ...
            {
                $Ctrl.Xaml.IO.NetworkSwitchAdapter.IsEnabled = $Ctrl.Xaml.IO.NetworkSwitchType.SelectedItem.Content -eq "External"
            })
    ...
    # Load the network panel with information
    ...
    Class VmTemplateNetwork
    {
        ...
        [String] ToString()
        {
            Return "<FEVirtual.VmTemplate[Network]>"
        }
    }
