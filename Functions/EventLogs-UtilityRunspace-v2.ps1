...
.LINK
.NOTES
          FileName: EventLogs-UtilityRunspaces.ps1
...
{
    [CmdLetBinding()]Param([Parameter(Mandatory)][Object]$Sync)
    ...
    Class ProjectController
    {
        ...
        {
            [Console]::WriteLine("Assign values from Hash Sync")
            ...
            # TabControl
            # ----------
            $This.MainTab                        = $Sync.MainTab           # Button
            $This.LogTab                         = $Sync.LogTab            # Button
            $This.OutputTab                      = $Sync.OutputTab         # Button
            $This.ViewTab                        = $Sync.ViewTab           # Button
                
            # Main Panel / Top      
            # ----------------      
            $This.MainPanel                      = $Sync.MainPanel         # Grid
            $This.Time                           = $Sync.Time              # TextBox
            $This.Start                          = $Sync.Start             # TextBox
            $This.Section                        = $Sync.Section           # ComboBox
            $This.Slot                           = $Sync.Slot              # ComboBox
            $This.Throttle                       = $Sync.Throttle          # ComboBox
            $This.AutoThrottle                   = $Sync.AutoThrottle      # CheckBox
            $This.Threads                        = $Sync.Threads           # ComboBox
                
            # Main Panel / Middle       
            # -------------------       
            $This.DisplayName                    = $Sync.DisplayName       # TextBox
            $This.Guid                           = $Sync.Guid              # TextBox
            $This.System                         = $Sync.System            # DataGrid
            $This.Archive                        = $Sync.Archive           # DataGrid
            $This.Base                           = $Sync.Base              # ComboBox
            $This.Browse                         = $Sync.Browse            # Button
            $This.Export                         = $Sync.Export            # Button
            $This.Mode                           = $Sync.Mode              # ComboBox
            $This.Continue                       = $Sync.Continue          # Button
            $This.Progress                       = $Sync.Progress          # Progress
               
            # Main Panel / Console   
            # --------------------   
            $This.ConsoleSet                     = $Sync.ConsoleSet        # Button
            $This.ConsoleSlot                    = $Sync.ConsoleSlot       # Table
            $This.Console                        = $Sync.Console           # Control
                
            # Main Panel / Table   
            # ------------------   
            $This.TableSet                       = $Sync.TableSet          # Button
            $This.TableSlot                      = $Sync.TableSlot         # Table
            $This.Table                          = $Sync.Table             # Control
                
            # Log Panel / Top   
            # ---------------   
            $This.LogPanel                       = $Sync.LogPanel          # Grid
            $This.LogMainProperty                = $Sync.LogMainProperty   # ComboBox
            $This.LogMainFilter                  = $Sync.LogMainFilter     # TextBox
            $This.LogMainRefresh                 = $Sync.LogMainRefresh    # Button
            $This.LogMainResult                  = $Sync.LogMainResult     # DataGrid
               
            # Log Panel / Mid-Anchor   
            # ------------------   
            $This.LogSelected                    = $Sync.LogSelected       # TextBox
            $This.LogTotal                       = $Sync.LogTotal          # TextBox
            $This.LogClear                       = $Sync.LogClear          # Button
                
            # Log Panel / Bottom   
            # ------------------   
            $This.LogOutputProperty              = $Sync.LogOutputProperty # ComboBox
            $This.LogOutputFilter                = $Sync.LogOutputFilter   # Textbox
            $This.LogOutputRefresh               = $Sync.LogOutputRefresh  # Button
            $This.LogOutputResult                = $Sync.LogOutputResult   # DataGrid
                
            # Output Panel      
            # ------------      
            $This.OutputPanel                    = $Sync.OutputPanel       # Grid
            $This.OutputProperty                 = $Sync.OutputProperty    # ComboBox
            $This.OutputFilter                   = $Sync.OutputFilter      # TextBox
            $This.OutputRefresh                  = $Sync.OutputRefresh     # Button
            $This.OutputResult                   = $Sync.OutputResult      # DataGrid
             
            # View Panel     
            # ----------     
            $This.ViewPanel                      = $Sync.ViewPanel         # Grid
            $This.ViewResult                     = $Sync.ViewResult        # DataGrid
            $This.ViewCopy                       = $Sync.ViewCopy          # Button
            $This.ViewClear                      = $Sync.ViewClear         # Button
        }
        Init([Object]$Sync)
        {
            ...
            $This.Time.IsEnabled                 = 0
            $This.Start.IsEnabled                = 0
            $This.DisplayName.IsEnabled          = 0
            $This.Guid.IsEnabled                 = 0
   
            # [System] ComboBoxes (Disabled)   
            $This.Section.IsEnabled              = 0
            $This.Slot.IsEnabled                 = 0
   
            # [Pseudo TabControl] Buttons (Disabled)
            $This.LogTab.IsEnabled               = 0
            $This.OutputTab.IsEnabled            = 0
            $This.ViewTab.IsEnabled              = 0
   
            # [Throttle/Threading] (Enabled)   
            $This.Throttle.IsEnabled             = 1 
            $This.AutoThrottle.IsEnabled         = 1
   
            $This.Reset( $This.Throttle.Items    , $Sync.ThreadControl.Index.Output )
            $This.Throttle.SelectedIndex         = $Sync.ThreadControl.Throttle.Index
   
            $This.Reset( $This.Threads.Items     , $Sync.ThreadControl.Index.Total )
            $This.Threads.SelectedIndex          = 0
            $This.Threads.IsEnabled              = 0
   
            # [Archive] Controls (Enabled)   
            $This.Archive.IsEnabled              = 1
            $This.Base.IsEnabled                 = 1
            $This.Mode.IsEnabled                 = 1
        ...
        MainTabEvent()
        {
            ...
            $This.Mode.Add_SelectionChanged(
            {
                Switch ($Ctrl.Mode.SelectedIndex)
                {
                    0
                    {
                        $Ctrl.Export.Visibility  = "Visible"
                        $Ctrl.Export.IsEnabled   = 0
                        $Ctrl.Browse.Visibility  = "Collapsed"
                        $Ctrl.Browse.IsEnabled   = 0
                        $Ctrl.Continue.IsEnabled = 1
                    }
                    1
                    {
                        $Ctrl.Export.Visibility  = "Collapsed"
                        $Ctrl.Export.IsEnabled   = 0
                        $Ctrl.Browse.Visibility  = "Visible"
                        $Ctrl.Browse.IsEnabled   = 1
                        $Ctrl.Continue.IsEnabled = 0
                    }
                }
    
                $Ctrl.Base.Items.Clear()
                If ($Ctrl.Archive)
                {
                    $Ctrl.ClearArchive()
                }
            ...
            $This.Section.Add_SelectionChanged(
            {
                $Index                           = $Ctrl.Section.SelectedIndex
            ...
            $This.Slot.Add_SelectionChanged(
            {
                $Index                           = $Ctrl.Slot.SelectedIndex
                If ($Ctrl.System -and $Index -gt -1)
                {
                    $Ctrl.SetSlot()
                }
            ...
            $This.AutoThrottle.Add_Checked(
            {
                $Ctrl.Throttle.IsEnabled         = 0
                $Ctrl.AutoThrottle()
            })
            
            $This.AutoThrottle.Add_Unchecked(
            {
                $Ctrl.Throttle.IsEnabled         = 1
            })
            
            $This.Throttle.Add_SelectionChanged(
            {
                $Item                            = $Ctrl.Throttle.SelectedItem
            ...
            $This.Browse.Add_Click(
            {
                ...
                Else
                {
                    $Ctrl.Base.Items.Add($Item.Filename)
                    $Ctrl.Base.SelectedIndex     = 0
                    $Ctrl.Browse.IsEnabled       = 0
                    $Ctrl.Continue.IsEnabled     = 1
            ...
            $This.Export.Add_Click(
            {
                ...
                Else
                {
                    $Ctrl.Base.Text              = $Item.SelectedPath
                }
            })
        
            $This.ConsoleSet.Add_Click(
            {
                If ($Ctrl.Subset -ne 0)
                {
                    $Ctrl.SubMain(0)
        ...
        LogTabEvent()
        {
            ...
            $This.LogMainFilter.Add_TextChanged( 
            {
                Start-Sleep -Milliseconds 50
                $Ctrl.Event.LogMain.Filter       = $Ctrl.LogMainFilter.Text
                If ($Ctrl.Project.Logs.Count -gt 0 -and $Ctrl.Event.LogMain.Filter -ne "")
                {
                    $Result = @( $Ctrl.Project.Logs | ? $Ctrl.LogProperty($Ctrl.LogMainProperty.Text) -match $Ctrl.CleanString($Ctrl.Event.LogMain.Filter) )
                    If ($Result)
                    {
                        $Ctrl.Reset($Ctrl.LogMainResult.Items,$Result)
                    }
                    Else
                    {
                        $Ctrl.LogMainResult.Items.Clear()
                    }
                }
                Else
                {
                    If ($Ctrl.Project.Logs.Count -notmatch $Ctrl.LogMainResult.Items.Count)
                    {
                        $Ctrl.Reset($Ctrl.LogMainResult.Items,$Ctrl.Project.Logs)
                    }
                }
            })
        
            [Console]::WriteLine("Log Main Refresh")
            $This.LogMainRefresh.Add_Click(
            {
                $Ctrl.Reset($Ctrl.LogMainResult.Items,$Ctrl.Project.Logs)
            })
        
            $This.LogMainResult.Add_SelectionChanged(
            {
                If ($Ctrl.LogMainResult.SelectedIndex -gt -1)
                {
                    $Ctrl.Event.LogMain.Index     = $Ctrl.LogMainResult.SelectedItem.Rank
                    $Ctrl.LogSelected.Text        = $Ctrl.Project.Logs[$Ctrl.Event.LogMain.Index].LogName
                    $Ctrl.LogTotal.Text           = $Ctrl.Project.Logs[$Ctrl.Event.LogMain.Index].Total
                    $Ctrl.GetLogOutput()
                }
            })
        
            $This.LogMainResult.Add_MouseDoubleClick(
            {
                $Ctrl.Event.LogMain.Index         = $Ctrl.LogMainResult.SelectedItem.Rank
                If ($Ctrl.Event.LogMain.Index -gt -1)
                {
                    $Ctrl.Reset($Ctrl.ViewResult.Items,$Ctrl.ViewProperties($Ctrl.Project.Logs[$Ctrl.Event.LogMain.Index].Config()))
                    $Ctrl.Main(3)
                }
            })
        
            $This.LogClear.Add_Click(
            {
                $Ctrl.Event.LogMain.Index         = -1
                $Ctrl.LogOutputResult.Items.Clear()
                $Ctrl.LogSelected.Text            = ""
                $Ctrl.LogTotal.Text               = 0
                $Ctrl.LogOutputProperty.IsEnabled = 0
                $Ctrl.LogOutputFilter.Text        = ""
                $Ctrl.LogOutputFilter.IsEnabled   = 0
                $Ctrl.LogOutputRefresh.IsEnabled  = 0
                $Ctrl.LogOutputResult.IsEnabled   = 0
            })
        
            $This.LogOutputFilter.Add_TextChanged(
            {
                Start-Sleep -Milliseconds 50
                $Ctrl.Event.LogOutput.Filter      = $Ctrl.LogOutputFilter.Text
                $Anchor                           = $Ctrl.Project.Logs[$Ctrl.Event.LogMain.Index].Output
                If ($Anchor.Count -gt 0 -and $Ctrl.LogOutputFilter -ne "")
                {
                    $Result = @( $Ctrl.Project.Logs[$Ctrl.Event.LogMain.Index].Output | ? $Ctrl.LogOutputProperty.Text -match $Ctrl.CleanString($Ctrl.Event.LogOutput.Filter) )
                    If ($Result)
                    {
                        $Ctrl.Reset($Ctrl.LogOutputResult.Items,$Result)
                    }
                    Else
                    {
                        $Ctrl.LogOutputResult.Items.Clear()
                ...
                Else
                {
                    ...
                    {
                        $Ctrl.Reset($Ctrl.LogOutputResult.Items,$Anchor)
                    }
                }
            })
        
            $This.LogOutputRefresh.Add_Click(
            {
                $Ctrl.Reset($Ctrl.LogOutputResult.Items,($Ctrl.Project.Logs | ? LogName -eq $Ctrl.LogSelected.Text | % Output))
            })
        
            $This.Xaml.IO.LogOutputResult.Add_MouseDoubleClick(
            {
                $Ctrl.Event.LogOutput.Index      = $Ctrl.LogOutputResult.SelectedItem.Index
                If ($Ctrl.Event.LogOutput.Index -gt -1)
                {
                    $Ctrl.Reset($Ctrl.ViewResult.Items,$Ctrl.ViewProperties($Ctrl.Project.Logs[$Ctrl.Event.LogMain.Index].Output[$Ctrl.Event.LogOutput.Index].Config()))
                    $Ctrl.Main(3)
                }
            })
        ...
        OutputTabEvent()
        {
            ...
            $This.OutputFilter.Add_TextChanged(
            {
                Start-Sleep -Milliseconds 50
                $Ctrl.Event.Output.Filter        = $Ctrl.OutputFilter.Text
                If ($Ctrl.Project.Output.Count -gt 0 -and $Ctrl.Event.Output.Filter -ne "")
                {
                    $Result = @( $Ctrl.Event.Output.Hash[0..($Ctrl.Event.Output.Hash.Count-1)] | ? $Ctrl.OutputProperty.Text -match $Ctrl.CleanString($Ctrl.Event.Output.Filter) )
                    If ($Result)
                    {
                        $Ctrl.Reset($Ctrl.OutputResult.Items,$Result)
                    }
                    Else
                    {
                        $Ctrl.OutputResult.Items.Clear()
                    }
                }
                Else
                {
                    If ($Ctrl.Event.Output.Count -notmatch $Ctrl.OutputResult.Items.Count)
                    {
                        $Ctrl.Reset($Ctrl.OutputResult.Items,$Ctrl.Project.Output)
                    }
                }
            })
        
            $This.OutputRefresh.Add_Click(
            {
                $Ctrl.Reset($Ctrl.OutputResult.Items,$Ctrl.Project.Output)
            })
        
            $This.OutputResult.Add_MouseDoubleClick(
            {
                $Ctrl.Event.Output.Index         = $Ctrl.OutputResult.SelectedIndex
                If ($Ctrl.OutputIndex -gt -1)
                {
                    $Ctrl.Reset($Ctrl.ViewResult.Items,$Ctrl.ViewProperties($Ctrl.Project.Output[$Ctrl.Event.Output.Index].Config()))
                    $Ctrl.Main(3)
                }
            })
            [Console]::WriteLine("End output tab")
        }
        ViewTabEvent()
        {
            ...
            $This.ViewCopy.Add_Click(
            {
                $Ctrl.ViewCopy() | Set-Clipboard
            })
        
            $This.ViewClear.Add_Click(
            {
                $Ctrl.ViewClear()
            })
            [Console]::WriteLine("End view tab")
        }
...
$ThreadControl                  = Get-ThreadController -Reserve 1
$Sync.ThreadControl             = $ThreadControl
$Sync.ThreadSet                 = $Null
$Sync.Thread                    = $Null
$Sync.Window                    = $Null
$Sync.Names                     = $Null
...
ForEach ($Function in (("Get-ThreadController Get-AssemblyList Get-SystemDetails {0}Item {0}Object {1}ConfigExtension {1}RecordExtension "+
"{1}Archive {1}Project {1}Xaml {1}Controller") -f "Get-Property","Get-EventLog" -Split " "))
{
...
$Thread0.PowerShell.AddScript(
{
    ...
    $Sync.Names      = [Regex]::Matches($Xaml,"(\s*Name\s*=\s*('|`")\w+)").Value -Replace "\s*Name=\`"","" | Select-Object -Unique
    $Sync.Types      = @( )
    $Sync.Window     = [Windows.Markup.XamlReader]::Load($Node)
    ForEach ($Name in $Sync.Names)
    {
        $Item        = $Sync.Window.FindName($Name)
        $Sync.Add($Name,$Item)
        $Sync.Types += Get-PropertyItem -Name $Name -Value $Item.GetType().Name
    }

    $IO              = Get-EventLogController -Sync $Sync
