...
namespace FightingEntropy
{
    namespace Format
    {
        ...
        public struct ByteSize
        {
            ...
            public ByteSize(string name, ulong bytes)
            {
                ...
                if (bytes < 870UL)
                    Unit = "Byte";
                else if (bytes < 891289UL)
                    Unit = "Kilobyte";
                else if (bytes < 912680550UL)
                    Unit = "Megabyte";
                else if (bytes < 934584883609UL)
                    Unit = "Gigabyte";
                else
                    Unit = "Terabyte";
    ...
    namespace Core
    {
        ...
        namespace Console
        {
            ...
            public class Controller
            {
                ...
                public void Update(int state, string status)
                {
                    ...
                    Output.Add(Status);
                    if (Mode == ConsoleMode.Full)
                    {
                        // Full
                        System.Console.WriteLine(Last());
                    }
                    else if (Mode == ConsoleMode.String)
                    {
                        // String
        ...
        namespace Node
        {
            ...
            public class Network
            {
                ...
                public Network(object network, object node)
                {
                    Index     = Node.Index;
                    Switch    = Network.Interface.Name;
                    IpAddress = Node.IpAddress;
                    Domain    = Node.Domain;
                    NetBios   = Node.NetBios;
            ...
            namespace Script
            {
                ...
                public class Block
                {
                    ...
                    {
                        Content.Add(new Line((uint)Content.Count, line));
                    }
                    public override string ToString()
                    {
                        return DisplayName;
                ...
                public class Controller
                {
                    ...
                    {
                        Block.Add((uint)Block.Count, phase, name, displayName, timeout, content);
                    }
            ...
            // ---                         ---
            // VmNetworkNode class         
            // VmNodeScriptBlockController Script.Controller
            // SocketTcpServer             
            // VmNodeSmbShare              
        }
    }
    ...
    namespace Windows
    {
        namespace Platform
        {
            ...
            public class Partition
            {
                ...
                public Partition(ManagementObject p)
                {
                    ...
                    Size      = new Format.ByteSize("Size", (ulong)raw);
                }
                public override string ToString()
                {
            ...
            public class Volume
            {
                ...
                public Volume(ManagementObject drive)
                {
                    ...
                    Size         = new Format.ByteSize("Size", (ulong)size);
                }
                public override string ToString()
                {
                    // Example: "[C:\100 GB]"
            ...
            public class Query
            {
                ...
                public Query(QueryType type, string target, string filter, params object[] arguments)
                {
            ...
            {
                object Query(string wmistring);
            }

            public interface ICimProvider
            {
                object Query(string className, string filter);
            }

            public interface INativeProvider
            {
                object Execute(string operation, object[] args);
            }
            ...
            public sealed class WmiProvider : IWmiProvider
            {
                ...
                public WmiProvider(string host)
                {
                    _host = string.IsNullOrWhiteSpace(host) ? "." : host;
                }
                public object Query(string wmiString)
                {
                    if (string.IsNullOrWhiteSpace(wmiString))
                        throw new ArgumentException("WMI string cannot be null or empty.", nameof(wmiString));

                    // If caller didn't specify host and we have a remote host, inject it
                    string input = wmiString;
                    if (!wmiString.StartsWith("\\\\") && !_host.Equals(".", StringComparison.OrdinalIgnoreCase))
                        input = "\\\\" + _host + "\\" + wmiString;

                    var instr = new Management.Instrumentation(input);

                    // Return the list of Instance objects; PowerShell can enumerate them
                    return instr.Instance;
                }
            }

            public sealed class CimProvider : ICimProvider
            {
                private readonly string _host;
                public CimProvider(string host)
                {
                    _host = host;
                }
                public object Query(string className, string filter)
                {
                    if (string.IsNullOrWhiteSpace(className))
                        throw new ArgumentException("Class name cannot be null or empty.", nameof(className));

                    string ns = ResolveNamespace(className);
                    ...
                    if (_host.Equals("localhost", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        session = CimSession.Create(_host, dcom);
                    }
                    else
                    {
                        var wsman = new WSManSessionOptions();
                        session = CimSession.Create(_host, wsman);
                    }
                    ...
                    using (session)
                    {
                        ...
                        var results = session.QueryInstances(ns, "WQL", query).ToList();
                        return results;
                    }
                }
                public string ResolveNamespace(string className)
                {
                    if (className.StartsWith("MSFT_", StringComparison.OrdinalIgnoreCase))
                    {
                        // Networking classes
                        if (className.StartsWith("MSFT_Net", StringComparison.OrdinalIgnoreCase))
                            return @"root\StandardCimv2";

                        // SMB classes
                        if (className.StartsWith("MSFT_Smb", StringComparison.OrdinalIgnoreCase))
                            return @"root\Microsoft\Windows\SMB";

                        // Storage classes (default MSFT_ bucket)
                        return @"root\Microsoft\Windows\Storage";
                    }

                    // Win32_* and everything else
                    return @"root\cimv2";
                }
            }

            public sealed class NativeProvider : INativeProvider
            {
                private readonly string _host;
                public NativeProvider(string host)
                {
                    _host = host;
                }
                public object Execute(string operation, object[] args)
                {
                    ...
                    if (operation == "MountDiskImage")
                    {
                        if (args.Length < 1 || !(args[0] is string))
                            throw new ArgumentException("MountDiskImage requires a path argument.");

                        return NativeDiskImage.Mount((string)args[0]);
                    }

                    if (operation == "DismountDiskImage")
                    {
                        if (args.Length < 1 || !(args[0] is string))
                            throw new ArgumentException("DismountDiskImage requires a path argument.");

                        return NativeDiskImage.Dismount((string)args[0]);
                    }

                    if (operation == "GetProcesses")
                    {
                        return NativeProcess.GetAll();
                    }

                    throw new NotSupportedException("Native operation not supported: " + operation);
                }
            }

            internal static class NativeDiskImage
            {
                public static object Mount(string path)
                {
                    // TODO: P/Invoke virtdisk.dll or managed wrapper
                    throw new NotImplementedException();
                }

                public static object Dismount(string path)
                {
                    // TODO: P/Invoke virtdisk.dll
                    throw new NotImplementedException();
                }
            }
            ...
            public class Controller
            {
                ...
                private readonly ICimProvider       _cim;
                private readonly INativeProvider _native;
                public Controller(string hostname)
                {
                    Hostname = string.IsNullOrWhiteSpace(hostname) ? "localhost" : hostname;

                    _wmi     = new WmiProvider(Hostname);
                    _cim     = new CimProvider(Hostname);
                    _native  = new NativeProvider(Hostname);
                }
                public object Query(string provider, string target, string filter)
                {
                    return Query(provider, target, filter, null);
                }
                public object Query(string provider, string target, string filter, string args)
                {
                    ...
                    if (p == "wmi")
                        ...
                        throw new ArgumentException("Exception [!] Unknown provider: " + provider);
                    ...
                    else
                    {
                        ...
                        argList = new object[parts.Length];
                        for (int i = 0; i < parts.Length; i++)
                            argList[i] = parts[i];
                    }

                    Query q = new Query(type, target, filter, argList);
                ...
                public object Query(Query query)
                {
                    ...
                    switch (query.Type)
                    {
                        case QueryType.Wmi    : return _wmi.Query(query.Target);
                        case QueryType.Cim    : return _cim.Query(query.Target, query.Filter);
                        case QueryType.Native : return _native.Execute(query.Target, query.Arguments);
                        default               : throw new NotSupportedException("Exception [!] Unsupported query type: [" + query.Type + "]");
                    }
                }
                public object GetDiskInfo(bool modern = true)
                {
                    if (modern)
                        return _cim.Query("MSFT_Disk", null);

                    return _wmi.Query("Win32_DiskDrive");
                }
                public object GetNetworkAdapters(bool modern = true)
                {
                    if (modern)
                        return _cim.Query("MSFT_NetAdapter", null);

                    return _wmi.Query("Win32_NetworkAdapter");
                }
                public object MountDiskImage(string path)
                {
                    if (string.IsNullOrWhiteSpace(path))
                        throw new ArgumentException("Path cannot be null or empty.", nameof(path));

                    return _native.Execute("MountDiskImage", new object[] { path });
                }
                public object DismountDiskImage(string path)
                {
                    if (string.IsNullOrWhiteSpace(path))
                        throw new ArgumentException("Path cannot be null or empty.", nameof(path));

                    return _native.Execute("DismountDiskImage", new object[] { path });
                }
                public object GetProcesses()
                {
                    return _native.Execute("GetProcesses", Array.Empty<object>());
                }
            }
        }
        ...
        namespace FileSystem
        {
            ...
            public class Entry
            {
                ...
                public string                 Label;
                [System.Management.Automation.Hidden]
                public Format.ModDateTime?  Created;
                public Format.ModDateTime? Modified;
                public string                  Name;
                public Format.ByteSize         Size;
                public string             Extension;
                public string           DisplayName;
                ...
                public bool                  Exists;
                public byte[]                 Bytes;
                public Entry(uint index, RawEntry raw)
                {
                    Index     = index;
            ...
            public class DirectoryController
            {
                ...
                public string                 Filter;
                public DirectoryController(string fullname) : this(fullname, 2, false, null)
                {
                    
                }
                public DirectoryController(string fullname, uint mode) : this(fullname, mode, false, null)
                {
                    
                }
                public DirectoryController(string fullname, uint mode, bool recurse) : this(fullname, mode, recurse, null)
                {

                }
                public DirectoryController(string fullname, uint mode, bool recurse, string filter)
                {
                    Index    = 0;
                    Type     = "";
                    Label    = "";
                ...
                public void SetMode(uint mode) => Mode = mode;
                public void SetRecurse(bool recurse) => Recurse = recurse;
                public void SetFilter(string pattern) => Filter = pattern;
                public void Check()
                {
                ...
                public void Refresh()
                {
                    ...
                    if (!string.IsNullOrEmpty(Filter))
                    {
                        try
                        {
                            rx = new Regex(Filter, RegexOptions.IgnoreCase);
                        }
                        catch
                        {
                            rx = null;
                    ...
                    // Prepare enumeration + (directory/file) flags
                    bool    includeDirs = Mode == ScanMode.DirsOnly  || Mode == ScanMode.DirsAndFiles;
                    bool   includeFiles = Mode == ScanMode.FilesOnly || Mode == ScanMode.DirsAndFiles;

                    List<RawEntry> raw  = new DirectoryScan().Scan(Fullname, Recurse);
                    ...
                    foreach (var r in raw)
                    {
                        if (r.IsDirectory && includeDirs)
                        {
                            if (!r.IsReparse)
                            {
                                var e = new Entry((uint)Entry.Count, r);
        ...
        namespace Reflection
        {
            public class Entry
            {
                ...
                public object          PermissionSet;
                public bool           ReflectionOnly;
                public object        SecurityRuleSet;
                public Entry(uint index, Assembly asm)
                {
                    Index = index;
                ...
                public void Assign(Assembly asm)
                {
                    Clear();
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            if (split.Length > 1 && split[1].Contains("="))
                                Version = new Version(split[1].Split('=')[1].Trim());
                            else if (split.Length > 1)
                                Version = new Version(split[1].Trim());

                            Fullname            = asm.FullName;
                            CodeBase            = asm.CodeBase;
                            
                            foreach (var item in asm.CustomAttributes)
                                CustomAttributes.Add(item.ToString());
                            
                            foreach (var item in asm.DefinedTypes)
                                DefinedTypes.Add(item);

                            EntryPoint          = asm.EntryPoint;
                            EscapedCodeBase     = asm.EscapedCodeBase;
                            Evidence            = asm.Evidence;

                            foreach (var item in asm.GetExportedTypes())
                                ExportedTypes.Add(item);
                            ...
                            foreach (var item in asm.GetModules())
                                Modules.Add(item);

                            PermissionSet   = asm.PermissionSet;
                            ReflectionOnly  = asm.ReflectionOnly;
                            SecurityRuleSet = asm.SecurityRuleSet;
                        }
                    }
            ...
            public class Controller
            {
                ...
                public void Refresh()
                {
                    ...
                    Entry.Sort((a, b) => string.Compare(a.Name, b.Name, StringComparison.OrdinalIgnoreCase));

                    Rerank();
                }
                public void Add(Assembly asm)
                {
                    Entry.Add(new Entry((uint)Entry.Count, asm));
                }
                public void Rerank()
                {
                    uint x = 0;
        ...
        namespace Process
        {
            ...
            public class Template
            {
                ...
                public bool      RedirectStdErr = false;
                public bool       RedirectStdIn = false;
                public bool      CreateNoWindow = false;
                public Instance        Instance;
                public Template(uint index, string name, string filename, string arguments, string workingdirectory)
                {
                    Index            = index;
                    Name             = name;
                    FileName         = filename;
                ...
                public ProcessStartInfo New()
                {
                    return new System.Diagnostics.ProcessStartInfo();
                }
                public Instance Start()
                {
                    Process p = Process.Start(Build());
                    Instance  = new Instance(0, p.Id);
                    return Instance;
                }
                public bool Stop()
                {
            ...
            public class Instance
            {
                ...
                public Format.ByteSize WorkingSet;
                public Format.ByteSize    Private;
                public Format.ByteSize      Paged;
                public Format.ByteSize   Nonpaged;
                public TimeSpan           CpuTime;
                public IntPtr          MainWindow;
                public Instance(bool flag, uint index, string name, int pid)
                {
                    Index = index;
                    Name  = name;
                    Id    = pid;
                ...
                public Instance(uint index, Process process)
                {
                    Index = index;
                ...
                public Instance(uint index, int pid)
                {
                    Index = index;
                    Id    = pid;
                    Refresh();
                ...
                public void Refresh(Process p)
                {
                    ...
                    try
                    {
                        WorkingSet = new Format.ByteSize("WorkingSet", (ulong)p.WorkingSet64              );
                        Private    = new Format.ByteSize("Private",    (ulong)p.PrivateMemorySize64       );
                        Paged      = new Format.ByteSize("Paged",      (ulong)p.PagedMemorySize64         );
                        Nonpaged   = new Format.ByteSize("Nonpaged",   (ulong)p.NonpagedSystemMemorySize64);
                ...
                {
                    // Memory
                    WorkingSet = new Format.ByteSize("WorkingSet", 0UL);
                    Private    = new Format.ByteSize("Private"   , 0UL);
                    Paged      = new Format.ByteSize("Paged"     , 0UL);
                    Nonpaged   = new Format.ByteSize("Nonpaged"  , 0UL);
            ...
            public class Controller
            {
                ...
                public static extern uint GetProcessId(IntPtr hWnd, out uint pid);
                ...
                public void Refresh()
                {
                    ...
                    foreach (System.Diagnostics.Process instance in System.Diagnostics.Process.GetProcesses())
                    {
                        try
                        {
                            if (!SafeCanQueryProcess(instance))
                                continue;
                            
                            Instance.Add(new Instance((uint)Instance.Count, instance));
                        }
                        catch
                        {
                            string name = "";
                            ...
                            Instance.Add(new Instance(false, (uint)Instance.Count, name, id));
                        }
                    }

                    SortBy("workingset", true);
                }
                public void SortBy(string property, bool ascending)
                {
                    string type = property.ToLower();

                    Comparison<Instance> cmp;

                    if (type == "id")
                    {
                        cmp = delegate(Instance a, Instance b)
                        {
                    ...
                    else if (type == "workingset")
                    {
                        cmp = delegate(Instance a, Instance b)
                        {
                            if (a.WorkingSet.Bytes < b.WorkingSet.Bytes) return -1;
                            if (a.WorkingSet.Bytes > b.WorkingSet.Bytes) return 1;
                            return 0;
                        };
                    }
                    else if (type == "private")
                    {
                        cmp = delegate(Instance a, Instance b)
                        {
                            if (a.Private.Bytes < b.Private.Bytes) return -1;
                            if (a.Private.Bytes > b.Private.Bytes) return 1;
                            return 0;
                        };
                    }
                    else if (type == "paged")
                    {
                        cmp = delegate(Instance a, Instance b)
                        {
                            if (a.Paged.Bytes < b.Paged.Bytes) return -1;
                            if (a.Paged.Bytes > b.Paged.Bytes) return 1;
                            return 0;
                        };
                    }
                    else if (type == "nonpaged")
                    {
                        cmp = delegate(Instance a, Instance b)
                        {
                            if (a.Nonpaged.Bytes < b.Nonpaged.Bytes) return -1;
                            if (a.Nonpaged.Bytes > b.Nonpaged.Bytes) return 1;
                            return 0;
                        };
                    }
                    ...
                    {
                        // Descending
                        Instance.Sort(delegate(Instance a, Instance b)
                        {
                            return cmp(b, a);
                        });
                    }
                ...
                public void Rerank()
                {
                    ...
                    {
                        Instance[x].Index = (uint)x;
                    }
                }
                public void Add(string name, string filename, string arguments, string workingdirectory)
                {
                    Template.Add(new Template((uint)Template.Count, name, filename, arguments, workingdirectory));
                }
                public static Instance FromPid(int pid)
                {
                    return new Instance(0, pid);
                }
                public static Instance FromHwnd(IntPtr hwnd)
                {
                    if (!IsWindow(hwnd))
                        return null;
                    ...
                    return new Instance(0, (int)pid);
                }
                private static bool SafeCanQueryProcess(Process p)
                {
                    try
                    {
        ...
        namespace Service
        {
            ...
            public class Entry
            {
                ...
                public Entry(uint index, RawEntry raw)
                {
                    ...
                    PathName         = raw.BinaryPath;
                    Description      = string.Empty;
                    DelayedAutoStart = false;

                    StateCode        = raw.StateCode;
                    StartTypeCode    = raw.StartTypeCode;
                    StartMode        = (StartMode)StartTypeCode;
                ...
                {
                    var m       = Regex.Match(Name, "^(.+)_([A-Za-z0-9]{3,12})$");

                    BaseName    = m.Success ? m.Groups[1].Value : Name;
                    InstanceTag = m.Success ? m.Groups[2].Value : "";
                }

                public void SetStartMode(StartMode mode) => StartMode = mode;
                public void SetState(State state) => State = state;
                public void SetStatus() => Status = string.Format("[Service]: {0} {1}", State, Name);
                public override string ToString()
                {
                    return base.ToString();
                }
            ...
            public class Svc
            {
                ...
                public static extern IntPtr OpenManager(string host, string db, uint access);

                [DllImport("advapi32.dll", EntryPoint = "EnumServicesStatusExW", CharSet = CharSet.Unicode, SetLastError = true)]
                public static extern bool EnumServices(IntPtr mgr, int level, uint type, uint state, IntPtr buff, uint buffsize, out uint need, out uint count, ref uint resume, string group);

                [DllImport("advapi32.dll", EntryPoint = "OpenServiceW", CharSet = CharSet.Unicode, SetLastError = true)]
                public static extern IntPtr OpenService(IntPtr mgr, string name, uint access);

                [DllImport("advapi32.dll", EntryPoint = "QueryServiceConfigW", CharSet = CharSet.Unicode, SetLastError = true)]
                public static extern bool GetConfig(IntPtr svc, IntPtr buff, uint buffsize, out uint need);

                [DllImport("advapi32.dll", EntryPoint = "QueryServiceStatusEx", CharSet = CharSet.Unicode, SetLastError = true)]
                public static extern bool GetStatus(IntPtr svc, int level, IntPtr buff, uint buffsize, out uint need);

                [DllImport("advapi32.dll", EntryPoint = "CloseServiceHandle", SetLastError = true)]
                public static extern bool CloseService(IntPtr handle);
            }
            ...
            public class Controller
            {
                ...
                public void Refresh()
                {
                    ...
                    List<RawEntry> raw = ScanNative();

                    uint index = 0;
                    for (int i = 0; i < raw.Count; i++)
                    {
                        Entry e = new Entry(index, raw[i]);
                        e.SetStatus();
                        Entry.Add(e);
                        index ++;
                    }
                }
                public static string FormatState(uint code)
                {
                    switch (code)
                    {
                ...
                public List<RawEntry> ScanNative()
                {
                    var list = new List<RawEntry>();

                    IntPtr mgr = Svc.OpenManager(null, null, Svc.SC_MANAGER_ENUMERATE_SERVICE);
                    ...
                    Svc.EnumServices(mgr, Svc.SC_ENUM_PROCESS_INFO, Svc.SERVICE_WIN32 | Svc.SERVICE_DRIVER, Svc.SERVICE_STATE_ALL, IntPtr.Zero, 0, out need, out count, ref resume, null);

                    if (need == 0)
                    {
                        Svc.CloseService(mgr);
                        return list;
                    }

                    IntPtr buff = Marshal.AllocHGlobal((int)need);

                    resume = 0;
                    if (!Svc.EnumServices(mgr, Svc.SC_ENUM_PROCESS_INFO, Svc.SERVICE_WIN32 | Svc.SERVICE_DRIVER, Svc.SERVICE_STATE_ALL, buff, need, out need, out count, ref resume, null))
                    {
                    ...
                    int structSize = Marshal.SizeOf(typeof(Svc.ENUM_SERVICE_STATUS_PROCESS));
                    IntPtr ptr = buff;

                    for (int i = 0; i < count; i++)
                    {
                        var essp = Marshal.PtrToStructure<Svc.ENUM_SERVICE_STATUS_PROCESS>(ptr);
                        var  ssp = essp.ServiceStatusProcess;

                        string     name = Marshal.PtrToStringUni(essp.lpServiceName) ?? string.Empty;
                        string  display = Marshal.PtrToStringUni(essp.lpDisplayName)  ?? string.Empty;

                        var raw         = new RawEntry();

                        raw.Name          = name;
                        raw.DisplayName   = display;
                        raw.StateCode     = ssp.dwCurrentState;
                        raw.StartTypeCode = 0;
                        raw.BinaryPath    = "";
                        raw.IsDriver      = (ssp.dwServiceType & Svc.SERVICE_DRIVER) != 0;

                        IntPtr svc = Svc.OpenService(mgr, name, Svc.SERVICE_QUERY_CONFIG | Svc.SERVICE_QUERY_STATUS);

                        if (svc != IntPtr.Zero)
                        {
                            uint cfgNeed = 0;
                            Svc.GetConfig(svc, IntPtr.Zero, 0, out cfgNeed);

                            if (cfgNeed > 0)
                            {
                                IntPtr cfgPtr = Marshal.AllocHGlobal((int)cfgNeed);

                                if (Svc.GetConfig(svc, cfgPtr, cfgNeed, out cfgNeed))
                                {
                                    Svc.QUERY_SERVICE_CONFIG cfg = (Svc.QUERY_SERVICE_CONFIG)Marshal.PtrToStructure(cfgPtr, typeof(Svc.QUERY_SERVICE_CONFIG));

                                    raw.StartTypeCode = cfg.dwStartType;
                                    raw.BinaryPath    = Marshal.PtrToStringUni(cfg.lpBinaryPathName);
                                }

                                Marshal.FreeHGlobal(cfgPtr);
                            }

                            Svc.CloseService(svc);
                        }
        ...
        namespace Hardware
        {
            ...
            public class DeviceItem
            {
                ...
                public DeviceItem(uint index, string name, string xclass, string deviceid, string[] hardwareids)
                {
                    Index       = index;

                    if (!Enum.TryParse<DeviceCategory>(xclass, true, out var category))
            ...
            public static class Pnp
            {
                ...
                public const int Present              = 0x00000002;
                public const int All                  = 0x00000004;
                [StructLayout(LayoutKind.Sequential)]
                public struct Info
                {
                    public uint       Size;
            ...
            public class Controller
            {
                ...
                public void Clear()
                {
                    Device = new List<DeviceItem>();   
                }
                public void SortByDeviceId()
                {
                    if (Device != null)
                    {
                        Device.Sort(delegate (DeviceItem a, DeviceItem b)
                        {
                ...
                public void Rerank()
                {
                    for (int i = 0; i < Device.Count; i++)
                        Device[i].Index = (uint)i;
                }
                public void Refresh()
                {
                    Clear();

                    IntPtr handle = GetClassSet(IntPtr.Zero, null, IntPtr.Zero, Pnp.Present | Pnp.All);

                    if (handle == IntPtr.Zero)
                        throw new Exception("Exception [!] No devices found");

                    try
                    {
                        // Start loop
                        uint    index = 0;
                        var      data = new Pnp.Info();
                        ...
                        while (GetDevices(handle, index, ref data))
                        {
                            index ++;
                            ...
                            if (GetProperty(handle, ref data, Pnp.SPDRP_FRIENDLYNAME, out type, buffer, (uint)buffer.Length, out size))
                            {
                            ...
                            if (GetProperty(handle, ref data, Pnp.SPDRP_CLASS, out type, buffer, (uint)buffer.Length, out size))
                            {
                                cls = Encoding.Unicode.GetString(buffer).TrimEnd('\0');
                            }
                            ...
                            if (!GetProperty(handle, ref data, Pnp.SPDRP_HARDWAREID, out type, buffer, (uint)buffer.Length, out size))
                            {
                                hw = new string[0];
                            }
                            ...
                            while (pos < size - 1)
                            {
                                end = Array.IndexOf(buffer, (byte)0, pos);
                                if (end <= pos) break;

                                s = Encoding.Unicode.GetString(buffer, pos, end - pos);
                                list.Add(s);
                            ...
                            // DeviceId
                            GetDeviceId(handle, ref data, sb, sb.Capacity, out req);

                            Device.Add(new DeviceItem((uint)Device.Count, name, cls, sb.ToString(), hw));
                        }
                    }
                    finally
                    {
                ...
                [DllImport("setupapi.dll", EntryPoint = "SetupDiGetClassDevs", CharSet = CharSet.Auto, SetLastError = true)]
                public static extern IntPtr GetClassSet(IntPtr guid, string filter, IntPtr handle, int flags); 

                [DllImport("setupapi.dll", EntryPoint = "SetupDiEnumDeviceInfo", SetLastError = true)]
                public static extern bool GetDevices(IntPtr handle, uint index, ref Pnp.Info info);

                [DllImport("setupapi.dll", EntryPoint = "SetupDiGetDeviceRegistryProperty", CharSet = CharSet.Auto, SetLastError = true)]
                public static extern bool GetProperty(IntPtr handle, ref Pnp.Info info, uint prop, out uint type, byte[] bytes, uint buffer, out uint size);

                [DllImport("setupapi.dll", EntryPoint = "SetupDiGetDeviceInstanceId", CharSet = CharSet.Auto, SetLastError = true)]
                public static extern bool GetDeviceId(IntPtr handle, ref Pnp.Info info, StringBuilder id, int idsize, out int size);

                [DllImport("setupapi.dll", EntryPoint = "SetupDiDestroyDeviceInfoList", SetLastError = true)]
                public static extern bool ClearDevices(IntPtr handle);
                public override string ToString()
                {
                    return string.Format("<{0}>", base.ToString());
                }
            ...
            namespace Wlan
            {
                ...
                {		
                    ...
                    public static extern uint WlanOpenHandle(uint version, IntPtr reserved, out uint neg, out IntPtr client);

                    [DllImport("wlanapi.dll", EntryPoint = "WlanCloseHandle")]
                    public static extern uint WlanCloseHandle(IntPtr client, IntPtr reserved);
                    ...
                    public static extern uint WlanEnumInterfaces(IntPtr client, IntPtr reserved, out IntPtr intlist);
                    ...
                    public static extern uint WlanGetProfile(
                        ...
                        IntPtr                                   reserved, 
                        [MarshalAs(UnmanagedType.LPWStr)] out string  xml, 
                        ref uint                                    flags, 
                        out uint                                   access
                    );
                    ...
                    public static extern uint WlanReasonCodeToString(uint reason, uint buffer, StringBuilder builder, IntPtr reserved);
                    ...
                    public static extern uint WlanConnect(IntPtr client, ref Guid id, ref WLAN_CONNECTION_PARAMETERS param, IntPtr reserved);

                    [DllImport("wlanapi.dll", EntryPoint = "WlanDisconnect", SetLastError = true)]
                    public static extern uint WlanDisconnect(IntPtr hClientHandle, ref Guid interfaceGuid, IntPtr pReserved);
                    ...
                    public struct WLAN_PROFILE_INFO_LIST
                    {
                        ...
                        public WLAN_PROFILE_INFO_LIST(IntPtr list)
                        {
                            dwNumberOfItems = (uint)Marshal.ReadInt32(list);
                            dwIndex         = (uint)Marshal.ReadInt32(list, 4);
                            ProfileInfo     = new WLAN_PROFILE_INFO[dwNumberOfItems];

                            IntPtr listTemp = new IntPtr(list.ToInt64() + 8);

                            for (int i = 0; i < dwNumberOfItems; i++)
                            {
                                list           = new IntPtr(listTemp.ToInt64() + i * Marshal.SizeOf(typeof(WLAN_PROFILE_INFO)));
                                ProfileInfo[i] = (WLAN_PROFILE_INFO)Marshal.PtrToStructure(list, typeof(WLAN_PROFILE_INFO));
                            }
                        }
                    }
                    ...
                    public struct WLAN_AVAILABLE_NETWORK_LIST
                    {
                        ...
                        public WLAN_AVAILABLE_NETWORK_LIST(IntPtr list)
                        {
                            ...
                            for (int i = 0; i < dwNumberOfItems; i++)
                            {
                                IntPtr pWlanAvailableNetwork = new IntPtr(list.ToInt64() + i * Marshal.SizeOf(typeof(WLAN_AVAILABLE_NETWORK)) + 8);
                    ...
                    public struct WLAN_INTERFACE_INFO_LIST
                    {
                        ...
                        public WLAN_INTERFACE_INFO_LIST(IntPtr list)
                        {
                            dwNumberOfItems   = (uint)Marshal.ReadInt32(list);
                            dwIndex           = (uint)Marshal.ReadInt32(list, 4);
                            ...
                            for (int i = 0; i < dwNumberOfItems; i++)
                            {
                                list = new IntPtr(listTemp.ToInt64() + i * Marshal.SizeOf(typeof(WLAN_INTERFACE_INFO)));
                                wlanInterfaceInfo[i] = (WLAN_INTERFACE_INFO)Marshal.PtrToStructure(list, typeof(WLAN_INTERFACE_INFO));
                            }
                        }
                    }
                    ...
                    public static extern uint WlanScan(IntPtr client, ref Guid id, IntPtr pDot11Ssid, IntPtr pIeData, IntPtr pReserved);
                }
            }
        }
        ...
        namespace Network
        {
            ...
            public class NetworkHost
            {
                ...
                public NetworkHost(IPAddress ipaddress)
                {
                    if (ipaddress.AddressFamily == System.Net.Sockets.AddressFamily.InterNetwork)
                        ...
                        throw new ArgumentException("Unsupported address", nameof(ipaddress));
                }
                public void Test()
                {
                    string host;
                    ...
                    if (!string.IsNullOrEmpty(Hostname))
                        ...
                        throw new Exception("Exception [!] Invalid (hostname/IP)");
        
                    try
                    {
                        var result = new Ping().Send(host, 1000);
        
                        Source      = Environment.MachineName;
                        Destination = host;
        
                        if (result.Address.AddressFamily == System.Net.Sockets.AddressFamily.InterNetwork)
                        {
                            IPv4Address = result.Address;
                            IPv6Address = null;
                        }
                        else if (result.Address.AddressFamily == System.Net.Sockets.AddressFamily.InterNetworkV6)
                        {
                            IPv4Address = null;
                            IPv6Address = result.Address;
                        }
        
                        Bytes       = (uint)result.Buffer.Length;
                        Time        = (uint)result.RoundtripTime;
                        Connected   = result.Status == IPStatus.Success;
        
                        System.Console.WriteLine(string.Format("Connected [~] Target: {0}, Ping: {1} ms", Destination, Time));
                    }
                    catch
                    {
            ...
            public class BitsDownloader : INetworkDownloader
            {
                public uint         Index = 0;
                public DownloadType  Type = DownloadType.Bits;
                public string Description = "<Background Intelligent Transfer Service>";
                public void Download(string sourceUrl, string destinationPath)
                {
                    try
                    {
                        var mgr = new BackgroundCopyManager() as IBackgroundCopyManager;
        
                        Guid jobId;
                        IBackgroundCopyJob job;
        
                        mgr.CreateJob("FE-BITS-Download", BG_JOB_TYPE.DOWNLOAD, out jobId, out job);
        
                        job.AddFile(sourceUrl, destinationPath);
                        job.Resume();
                        ...
                        while (true)
                        {
                            ...
                            if (state == BG_JOB_STATE.TRANSFERRED)
                            {
                                job.Complete();
                                break;
                            }
        
                            if (state == BG_JOB_STATE.ERROR || state == BG_JOB_STATE.CANCELLED)
                                throw new Exception("Exception [!] Bits transfer failed");
                            else
            ...
            public class IrmDownloader : INetworkDownloader
            {
                public uint        Index = 1;
                public DownloadType Type = DownloadType.WebClient;
                public string Description = "<System.Net.WebClient>";
                public void Download(string sourceUrl, string destinationPath)
                {
                    try
                    {
                        ...
                        {
                            File.WriteAllBytes(destinationPath, client.DownloadData(sourceUrl));
                        }
                    }
                    catch (Exception ex)
                    {
                        throw new Exception("IRM download failed: " + ex.Message);
                    }
                }
            }
        
            public class HttpClientDownloader : INetworkDownloader
            {
                public uint        Index = 2;
                public DownloadType Type = DownloadType.HttpClient;
                public string Description = "<System.Net.Http.HttpClient>";
                public void Download(string sourceUrl, string destinationPath)
                {
                    try
                    {
                        ...
                        {
                            var task = client.GetByteArrayAsync(sourceUrl);
                            task.Wait();
        
                            File.WriteAllBytes(destinationPath, task.Result);
                        }
                    }
                    catch (Exception ex)
                    {
                        throw new Exception("HttpClient download failed: " + ex.Message);
                    } 
                }
            }
            ...
            public interface IBackgroundCopyManager
            {
                ...
                void EnumJobs(uint flags, [MarshalAs(UnmanagedType.Interface)] out IEnumBackgroundCopyJobs ppEnum);
            }
            ...
            public class Controller
            {
                ...
                public Hardware.NetworkAdapter Selected;
                public Security.ProtocolMode   Protocol;
                public bool                      Online;
                public bool               CanResolveDns;
                public bool            CanReachInternet;
                public NetworkHost               Target;
                public Controller()
                {
                ...
                public void Refresh()
                {
                    ...
                    // Automatically select one that has gateway/external network info
                    SelectAdapter();
        
                    // Automatically set target to github
                    if (Target == null)
                        Target = new NetworkHost(DefaultTargetHostname());
        
                    // Check the connection
                    CheckConnection();
        
                    // (Get/set) the current security protocol
                    GetCurrentSecurityProtocol();
        
                    if (Protocol != Security.ProtocolMode.Tls12)
                    {
                        SetSecurityProtocol(Security.ProtocolMode.Tls12);
                    }
                }
                public void SelectAdapter()
                {
                    ...
                    foreach (var adapter in Network.Output)
                    {
                        var iface = adapter.Interfaces.FirstOrDefault();
                        if (iface == null)
                            continue;
        
                        var cfg = iface.Configurations.FirstOrDefault(c => c.Protocol == ProtocolFamily.IPv4);
                        if (cfg == null)
                            continue;
        
                        if (!string.IsNullOrEmpty(cfg.Gateway))
                        {
                            Selected = adapter;
                            return;
                        }
                ...
                public void GetCurrentSecurityProtocol()
                {
                    Protocol                            = (Security.ProtocolMode)ServicePointManager.SecurityProtocol;
                }
                public void SetSecurityProtocol(uint mode)
                {
                    ServicePointManager.SecurityProtocol = (SecurityProtocolType)mode;
                    Protocol                             = (Security.ProtocolMode)mode;
                }
                public void SetSecurityProtocol(Security.ProtocolMode mode)
                {
                    ServicePointManager.SecurityProtocol = (SecurityProtocolType)mode;
                    Protocol                             = mode;
                }
                ...
                {
                    Target = new NetworkHost(hostname);
                }
                public void SetTargetIPAddress(string ipaddress)
                {
                    if (Target != null)
                    {
                        ...
                        {
                            Target.IPv4Address = IPAddress.Parse(ipaddress);
                        }
                        else if (Regex.IsMatch(ipaddress, PatternIPv6()))
                        {
                            Target.IPv6Address = IPAddress.Parse(ipaddress);
                        }
                    }
                }
                public string PatternIPv6()
                {
                    string[] input = new string[11];
        
                    input[00]      = @"(([0-9A-Fa-f]{1,4}:){7}[0-9A-Fa-f]{1,4}|";
                    input[01]      = @"([0-9A-Fa-f]{1,4}:){1,7}:|";
                    input[02]      = @"([0-9A-Fa-f]{1,4}:){1,6}:[0-9A-Fa-f]{1,4}|";
                    input[03]      = @"([0-9A-Fa-f]{1,4}:){1,5}(:[0-9A-Fa-f]{1,4}){1,2}|";
                    input[04]      = @"([0-9A-Fa-f]{1,4}:){1,4}(:[0-9A-Fa-f]{1,4}){1,3}|";
                    input[05]      = @"([0-9A-Fa-f]{1,4}:){1,3}(:[0-9A-Fa-f]{1,4}){1,4}|";
                    input[06]      = @"([0-9A-Fa-f]{1,4}:){1,2}(:[0-9A-Fa-f]{1,4}){1,5}|";
                    input[07]      = @"[0-9A-Fa-f]{1,4}:((:[0-9A-Fa-f]{1,4}){1,6})|";
                    input[08]      = @":((:[0-9A-Fa-f]{1,4}){1,7}|:)|";
                    input[09]      = @"([0-9A-Fa-f]{1,4}:){1,4}((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)\.){3}";
                    input[10]      = @"(25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d))";
                ...
                public string DefaultTargetIPAddress()
                {
                    return "1.1.1.1";
                }
                public void CheckConnection()
                {
                    ...
                    {
                        CanResolveDns    = TestDns(Target.Hostname);
                        CanReachInternet = TestPing(Target.Hostname);
                    }
                    else if (Target.IPv4Address != null)
                    {
                        CanResolveDns    = TestDns(Target.IPv4Address.ToString());
                        CanReachInternet = TestPing(Target.IPv4Address.ToString());
                    }
                    else if (Target.IPv6Address != null)
                    {
                        CanResolveDns    = TestDns(Target.IPv6Address.ToString());
                        CanReachInternet = TestPing(Target.IPv6Address.ToString());
                    }
        
                    Online = Target.Connected = CanResolveDns || CanReachInternet;
                }
                public void Test()
                {
                    if (Target == null)
                ...
                private bool TestDns(string host)
                {
                    ...
                    try
                    {
                        ...
                        return entry != null && entry.AddressList.Length > 0;
                    }
                    catch
                    {
                        return false;
                ...
                private bool TestPing(string host)
                {
                    try
                    {
        ...
        namespace Thread
        {
            ...
            {
                public uint         Index;
                public IntensityType Type;
                public string        Name;
                public string Description;
                public bool      Selected;
                public IntensitySlot(uint index, IntensityType type, string name, string description)
                {
                    Index       = index;
                    Type        = type;
                    Name        = name;
            ...
            public class Controller
            {
                ...
                public ConcurrentBag<object>                   Bag;
                public ConcurrentQueue<string>            Messages;
                public ConcurrentDictionary<int, Instance> Threads;
                public RunspacePool                           Pool;
                public List<IntensitySlot>               Intensity;
                private PerformanceCounter              cpuCounter;
                public List<IAsyncResult>                     Jobs;
                ...
                private void Prime()
                {
                    Intensity = new List<IntensitySlot>();

                    Intensity.Add(new IntensitySlot((uint)Intensity.Count, IntensityType.Low, "Low", "Minimum CPU usage, longest processing time"));
                    Intensity.Add(new IntensitySlot((uint)Intensity.Count, IntensityType.Medium, "Medium", "Balanced CPU usage, average processing time"));
                    Intensity.Add(new IntensitySlot((uint)Intensity.Count, IntensityType.High, "High", "Maximum CPU usage, shortest processing time"));
                }
                ...
                public void ComputeMaxThreads()
                {
                    ...
                    {
                        case 0  : calc = Math.Max(1, cores / 2)         ; break; // Low
                        case 1  : calc = Math.Max(1, cores)             ; break; // Medium
                        case 2  : calc = Math.Max(2, (int)(cores * 1.5)); break; // High
                        default : calc = Math.Max(1, cores)             ; break; // Default
                    }
                ...
                public void InitializePool()
                {
                    ...
                    foreach (var item in SessionState)
                    {
                        ...
                        switch (item.Type)
                        {
                            case SessionStateType.Assembly: iss.Variables.Add(new SessionStateVariableEntry(item.Name, item.Bytes, item.Description)); break;
                            case SessionStateType.Function: iss.Commands.Add(new SessionStateFunctionEntry(item.Name, item.GetString())); break;
                            case SessionStateType.Variable: iss.Variables.Add(new SessionStateVariableEntry(item.Name, item.Bytes, item.Description)); break;
                            case SessionStateType.Script  : iss.Commands.Add(new SessionStateFunctionEntry(item.Name, item.GetString())); break;
                            default: break;
                        }
                    }
        ...
        namespace Security
        {
            ...
            public class Identifier
            {
                ...
                public Identifier(string sidstring)
                {
                    try
                    {
                        Init(new SecurityIdentifier(sidstring));
                    }
                    catch
                    {
            ...
            public class CurrentIdentity
            {
                ...
                public CurrentIdentity(WindowsIdentity identity)
                {
                    if (identity == null)
                        return;
                ...
                public static CurrentIdentity GetCurrent()
                {
                    return new CurrentIdentity(WindowsIdentity.GetCurrent());
                }
                public override string ToString()
                {
                    return string.Format("<{0}>", Account != null ? Account.Name : base.ToString());
                }
            }

            public enum CertificateEntryType
            {
                Store       = 0,
                Certificate = 1,
            }
            ...
            {
                public uint                   Index;
                public CertificateEntryType    Type;
                public string                  Name;
                public string              Fullname;
                public bool                  Exists;
                ...
                public Format.ModDateTime NotBefore;
                public Format.ModDateTime  NotAfter;
                public bool           HasPrivateKey;
                public CertificateEntry(uint index, X509Store store)
                {
                    Index = index;
                    InitStore(store);
                }
                public CertificateEntry(uint index, X509Store store, X509Certificate2 cert)
                {
                    Index = index;
                    InitCertificate(store, cert);
                }
                private void InitStore(X509Store store)
                {
                    Type          = CertificateEntryType.Store;

                    StoreName     = store.Name;
                    StoreLocation = store.Location;
                    
                    Name          = store.Name;
                    Fullname      = string.Format("{0}\\{1}", store.Location, store.Name);

                    StoreExists();
                }
                private void InitCertificate(X509Store store, X509Certificate2 cert)
                {
                    Type          = CertificateEntryType.Certificate;
                    Certificate   = cert;
                    ...
                    Fullname      = string.Format("{0}\\{1}\\{2}", StoreLocation, StoreName, cert.Thumbprint);

                    Exists        = true;
                }
                private void StoreExists()
                {
                    if (Type == CertificateEntryType.Store)
                    {
                        try
                        {
            ...
            {
                public List<CertificateEntry> Entry;
        ...
        namespace Component
        {
            namespace Dism
            {
                ...
                [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Unicode)]
                public struct ProvisionedAppxPackage
                {
                    public string PackageName;
                    public string PublisherId;
                    public string Version;
                ...
                public abstract class DismObject
                {
                    public uint          Index { get; set; }
                    public DismObjectType Type { get; set; }
                    public string        Drive { get; set; }
                    public string         Name { get; set; }
                    public string     Fullname { get; set; }
                    public DismObject()
                    ...
                    public abstract void Refresh(Controller api);
                    public override string ToString()
                    {
                        return Fullname;
                    }
                ...
                public class DismDirectory : DismObject
                {
                    public List<DismObject> Children;
                    public DismDirectory(string drive, string name)
                    {
                        Type     = DismObjectType.Directory;
                        Drive    = drive;

                        if (string.IsNullOrEmpty(name))
                        {
                            Name     = "\\";
                            Fullname = string.Format("{0}\\", Drive);
                        }
                        else
                        {
                            Name     = name;
                            Fullname = string.Format("{0}\\{1}", Drive, name);
                        }

                        Children = new List<DismObject>();
                    }
                    public override void Refresh(Controller api)
                    {
                        // repopulated by Controller.Refresh()
                    }
                }

                public class DismFeature : DismObject
                {
                    public FeatureState State;
                    public DismFeature()
                    {
                        Type = DismObjectType.Feature;
                    }
                    public override void Refresh(Controller api)
                    {
                        // repopulated by Controller.Refresh()
                ...
                public class DismPackage : DismObject
                {
                    public PackageState State;
                    public DismPackage()
                    {
                        Type = DismObjectType.Package;
                    }
                    public override void Refresh(Controller api)
                    {
                        // repopulated by Controller.Refresh()
                ...
                public class DismCapability : DismObject
                {
                    public string Status;
                    public DismCapability()
                    {
                        Type = DismObjectType.Capability;
                    }
                    public override void Refresh(Controller api)
                    {
                        // repopulated by Controller.Refresh()
                ...
                public class DismDriver : DismObject
                {
                    public string ProviderName;
                    public DismDriver()
                    {
                        Type = DismObjectType.Driver;
                    }
                    public override void Refresh(Controller api)
                    {
                        // repopulated by Controller.Refresh()
                ...
                public class DismAppx : DismObject
                {
                    public string PublisherId;
                    public string Version;
                    public string Architecture;
                ...
                public class DismProvisionedAppx : DismObject
                {
                    public string PublisherId;
                    public string Version;
                    public string Architecture;
                    public DismProvisionedAppx()
                    {
                        Type = DismObjectType.ProvisionedAppx;
                    }
                    public override void Refresh(Controller api)
                    {
                        // repopulated by Controller.Refresh()
                ...
                internal static class Activation
                {
                    ...
                    [DllImport("kernel32.dll", SetLastError = true)]
                    private static extern void DeactivateActCtx(uint dwFlags, IntPtr lpCookie);

                    [DllImport("kernel32.dll", SetLastError = false)]
                    private static extern void ReleaseActCtx(IntPtr hActCtx);
                    private static string GetDismExePath()
                    {
                        string windir = Environment.GetFolderPath(Environment.SpecialFolder.Windows);
                        return Path.Combine(windir, "System32", "dism.exe");
                    }
                    public sealed class Scope : IDisposable
                    {
                        ...
                        public Scope()
                        {
                            var exe = GetDismExePath();
                            if (!File.Exists(exe))
                                throw new FileNotFoundException("dism.exe not found", exe);
                        ...
                        public void Dispose()
                        {
                            ...
                            if (_hActCtx != IntPtr.Zero && _hActCtx != new IntPtr(-1))
                            {
                ...
                public class Controller
                {
                    ...
                    public IntPtr LastSession { get; set; }
                    public Controller()
                    {
                        InitializeDefaults();
                    }
                    ...
                    {
                        Mode       = SessionMode.Online; // Default mode
                        Drive      = "Dism:";
                        Root       = new DismDirectory(Drive, "");

                        LogLevel   = 3;
                        LogPath    = DefaultLogPath();
                        ImagePath  = DefaultImagePath();
                        ScratchDir = DefaultScratchPath();
                    }
                    private string DefaultLogPath()
                    {
                        return Environment.GetEnvironmentVariable("SystemRoot") + "\\Logs\\Dism\\dism.log";
                    }
                    private string DefaultScratchPath()
                    {
                        return Environment.GetEnvironmentVariable("Temp") + "\\Dism";
                    }
                    private string DefaultImagePath()
                    {
                        return null; // Environment.GetEnvironmentVariable("SystemRoot");
                    }
                    public int InitializeApi()
                    {
                        ...
                        return DismInitialize(LogLevel, LogPath, ScratchDir);
                    }
                    public int ShutdownApi()
                    {
                        return DismShutdown();
                    }
                    public int OpenOnlineSessionApi()
                    {
                        IntPtr session;
                        var hr = DismOpenSession(null, null, null, out session); // imagepath, windowsdirectory, systemdrive, session
                    ...
                    public int CloseSessionApi()
                    {
                        if (Session == IntPtr.Zero)
                            return 0;

                        var hr = DismCloseSession(Session);
                        if (hr == 0)
                            Session = IntPtr.Zero;
                        
                        return hr;
                    }
                    public int TestOpenSession()
                    {
                        InitializeApi();

                        IntPtr session;
                        int hr = DismOpenSession(null, null, null, out session);
                    ...
                    public void OpenSession()
                    {
                        if (Session != IntPtr.Zero)
                        {
                            throw new Exception("Exception [!] Session already open");
                        }

                        using (new Activation.Scope())
                        {
                            int hrInit = InitializeApi();

                            if (hrInit != 0 && hrInit != 1)
                                throw new Exception(string.Format("Exception [!] DismApi.dll DismInitialize() => HRESULT 0x{0}", hrInit));
                            else
                                System.Console.WriteLine(string.Format("Success [+] DismApi.dll DismInitialize() => HRESULT 0x{0}", hrInit));

                            IntPtr session = IntPtr.Zero;
                            int hr;

                            System.Console.WriteLine("Loading [+] DismApi.dll DismOpenSession()");

                            if (Mode == SessionMode.Online)
                            {
                                const string DISM_ONLINE_IMAGE = "DISM_ONLINE_IMAGE";
                                hr = DismOpenSession(DISM_ONLINE_IMAGE, null, null, out session);

                                System.Console.WriteLine("Loading [+] DismApi.dll DismOpenSession() => Online/Handle: (" + hr + ")");
                            }
                            else
                            {
                                if (string.IsNullOrEmpty(ImagePath))
                                    throw new Exception("Exception [!] Image path required for offline mode");

                                if (!Directory.Exists(ImagePath))
                                    throw new DirectoryNotFoundException("Exception [!] Offline image not found");

                                hr = DismOpenSession(ImagePath, null, null, out session);

                                System.Console.WriteLine("Loading [+] DismApi.dll DismOpenSession() => Offline/Handle: (" + hr + ")");
                            }

                            if (hr != 0)
                                throw new Exception(string.Format("Exception [!] DismApi.dll DismOpenSession() => HRESULT 0x{0}", hr));

                            if (session == IntPtr.Zero)
                                throw new Exception("Exception [!] Session pointer is (null/zero)");
                    ...
                    private void BuildFeatures()
                    {
                        if (Session != IntPtr.Zero)
                        {
                            DismDirectory dir = new DismDirectory(Drive, "Features");

                            foreach (var f in GetFeatures())
                            {
                                dir.Children.Add(new DismFeature
                                {
                                    Name     = f.FeatureName,
                                    Fullname = string.Format("{0}\\{1}", dir.Fullname, f.FeatureName),
                                    State    = f.State 
                                });
                            }
                    ...
                    private void BuildPackages()
                    {
                        if (Session != IntPtr.Zero)
                        {
                            DismDirectory dir = new DismDirectory(Drive, "Packages");

                            foreach (var f in GetPackages())
                            {
                                dir.Children.Add(new DismPackage
                                {
                                    Name     = f.PackageName,
                                    Fullname = string.Format("{0}\\{1}", dir.Fullname, f.PackageName),
                                    State    = f.PackageState 
                                });
                            }
                    ...
                    private void BuildAppx()
                    {
                        if (Session != IntPtr.Zero)
                        {
                            DismDirectory dir = new DismDirectory(Drive, "Appx");

                            foreach (var f in GetAppxPackages())
                            {
                                dir.Children.Add(new DismAppx
                                {
                                    Name     = f.PackageName,
                                    Fullname = string.Format("{0}\\{1}", dir.Fullname, f.PackageName),
                                });
                    ...
                    private void BuildProvisionedAppx()
                    {
                        if (Session != IntPtr.Zero)
                        {
                            DismDirectory dir = new DismDirectory(Drive, "ProvisionedAppx");

                            foreach (var f in GetProvisionedAppxPackages())
                            {
                                dir.Children.Add(new DismAppx
                                {
                                    Name     = f.PackageName,
                    ...
                    public Feature[] GetFeatures()
                    {
                        ...
                        DismGetFeatures(Session, out ptr, out count);

                        return MarshalArray<Feature>(ptr, count);
                    }
                    public Package[] GetPackages()
                    {
                        ...
                        DismGetPackages(Session, out ptr, out count);

                        return MarshalArray<Package>(ptr, count);
                    }
                    public AppxPackage[] GetAppxPackages()
                    {
                        IntPtr ptr;
                        uint count;

                        DismGetAppxPackages(Session, out ptr, out count);

                        return MarshalArray<AppxPackage>(ptr, count);
                    }
                    public ProvisionedAppxPackage[] GetProvisionedAppxPackages()
                    {
                        IntPtr ptr;
                        uint count;
                        DismGetProvisionedAppxPackages(Session, out ptr, out count);

                        return MarshalArray<ProvisionedAppxPackage>(ptr, count);
                    }
                    public AppxPackageInfo GetAppxPackageInfo(string packageName)
                    {
                        IntPtr ptr;
                        DismGetAppxPackageInfo(Session, packageName, out ptr);

                        return (AppxPackageInfo)Marshal.PtrToStructure(ptr, typeof(AppxPackageInfo));
                    }
                    private T[] MarshalArray<T>(IntPtr ptr, uint count)
                    {
                        ...
                        for (int i = 0; i < count; i++)
                        {
                            IntPtr cur = new IntPtr(ptr.ToInt64() + (i * size));
                            arr[i] = (T)Marshal.PtrToStructure(cur, typeof(T));
                        }
                    ...
                    public static extern int DismInitialize(int logLevel, string logPath, string scratchDir);
                    ...
                    [DllImport("DismApi.dll", CharSet = CharSet.Unicode)]
                    public static extern int DismGetFeatures(IntPtr session, out IntPtr features, out uint count);

                    [DllImport("DismApi.dll", CharSet = CharSet.Unicode)]
                    public static extern int DismGetPackages(IntPtr session, out IntPtr packages, out uint count);
                    
                    [DllImport("DismApi.dll", CharSet = CharSet.Unicode)]
                    public static extern int DismGetAppxPackages(IntPtr session, out IntPtr appxPackages, out uint count);
                    
                    [DllImport("DismApi.dll", CharSet = CharSet.Unicode)]
                    public static extern int DismGetProvisionedAppxPackages(IntPtr session, out IntPtr appxPackages, out uint count);
                    
                    [DllImport("DismApi.dll", CharSet = CharSet.Unicode)]
                    public static extern int DismGetAppxPackageInfo(IntPtr session, string packageName, out IntPtr packageInfo);
                }
            }
        }
    ...
    namespace Module
    {
        public struct Template // Core module template
        {
            public string        Name;
        ...
        namespace Host
        {
            ...
            public class Property
            {
                ...
                public string   Name;
                public object  Value;
                public Property(uint index, string source, string name, object value)
                {
                    Index  = index;
                    Source = source;
            ...
            public class PropertySet
            {
                ...
                public PropertySet(uint index, string source, string command, int type, EngineIntrinsics context)
                {
                    Index    = index;
                    Source   = source;
                    Command  = command;
                ...
                {
                    Property.Add(new Property((uint)Property.Count, Source, name, value));
                }
                public void Clear()
                {
                    Property = new List<Property>();
                ...
                public void Sort()
                {
                    Property.Sort(delegate(Property a, Property b)
                    {
                        return string.Compare(a.Name, b.Name, StringComparison.OrdinalIgnoreCase);
                    });
                }
            ...
            public class Controller
            {
                ...
                public List<PropertySet>     Output;
                private EngineIntrinsics    Context;
                ...
                public PropertySet Source(string source)
                {
                    return Output.Where(delegate(PropertySet x)
                    { 
                        return String.Equals(x.Source, source, StringComparison.OrdinalIgnoreCase);
                    
                    }).FirstOrDefault();
                }
                public Property Property(string source, string name)
                {
                    PropertySet set = Source(source);

                    if (set == null || set.Property == null) return null;

                    return set.Property.Where(delegate(Property p)
                    {
                ...
                {
                    PropertySet set = Output[(int)index];
                    if (set != null)
                    {
                        set.Add(name, value);
                    }
                ...
                {
                    Output.Add(new PropertySet((uint)Output.Count, source, command, type, Context));
                }
                ...
                {
                    Property p = Property("PowerShell", "PSVersion");
        ...
        namespace Interop
        {
            ...
            public class Windows : Controller
            {
                public Windows.Registry.Controller Registry;
                public Windows.Toast.Controller       Toast;
                public Windows.Security.Controller Security;
                public Windows.Hardware.Controller Hardware;
                public Windows.Process.Controller   Process;
                public Windows.Network.Controller   Network;
                public Windows.Thread.Controller     Thread;
                public Windows()
                {
                ...
                {
                    if (Registry == null)
                        Registry = new Windows.Registry.Controller();
                }
                public override void LoadToast()
                {
                    if (Toast == null)
                        Toast = new Windows.Toast.Controller();
                }
                public override void LoadSecurity()
                {
                    if (Security == null)
                        Security = new Windows.Security.Controller();
                }
                public override void LoadHardware()
                {
                    if (Hardware == null)
                        Hardware = new Windows.Hardware.Controller();
                }
                public override void LoadProcess()
                {
                    if (Process == null)
                        Process = new Windows.Process.Controller();
                }
                public override void LoadNetwork()
                {
                    if (Network == null)
                        Network = new Windows.Network.Controller();
                }
                public override void LoadThread()
                {
                    if (Thread == null)
                        Thread = new Windows.Thread.Controller();
                }
            }

            public class Linux : Controller
            {
                public string    Registry;
                public string       Toast;
                ...
                public Linux()
                {
                    // Linux doesn't use the registry, must load using a config file
                    // Must also write a bunch of shit for linux interop
                }
                public override string Name => "Linux";
                public override void LoadRegistry()
                {
                    // if (Registry == null)
                    //     Registry = new Linux.Registry.Controller();
                }
                public override void LoadToast()
                {
                    // if (Toast == null)
                    //     Toast = new Linux.Toast.Controller();
                }
                public override void LoadSecurity()
                {
                    // if (Security == null)
                    //     Security = new Linux.Security.Controller();
                }
                public override void LoadHardware()
                {
                    // if (Hardware == null)
                    //     Hardware = new Linux.Hardware.Controller();
                }
                public override void LoadProcess()
                {
                    // if (Process == null)
                    //     Process = new Linux.Process.Controller();
                }
                public override void LoadNetwork()
                {
                    // if (Network == null)
                    //     Network = new Linux.Network.Controller();
                }
                public override void LoadThread()
                {
                    // if (Thread == null)
                    //     Thread = new Linux.Thread.Controller();
                }
            }
        }
        ...
        namespace Manifest
        {
            ...
            public struct Content : Format.Content { }
        ...
        public class Revision
        {
            ...
            {
                Match m = Regex.Match(line, Pattern(prop[type]));
        ...
        public class Controller
        {
            ...
            public Host.Controller                    Host;
            public Interop.Platform                Interop;
            public Root.Controller                    Root;
            public Manifest.Controller            Manifest;
            public Core.Console.Controller         Console;
            public Core.Theme.Controller             Theme;
            ...
            public void Prime()
            {
                // Primes all fields that are NOT independent

                Console.SetMode(0); // Object mode
                ...
                Write(2, 1, string.Format("Loaded [~] {0}[{1}]", DisplayName(), CurrentVersion()));
            ...
            {
                if (Console.Mode != FightingEntropy.Console.ConsoleMode.Silent)
                {
                    // Silent mode
                    Console.SetMode(2);
                }
                ...
                // WriteTrack(index)
                foreach (Theme.TemplateTrack track in Theme.Track)
                {
                ...
                if (Console.Mode != FightingEntropy.Console.ConsoleMode.Full)
                {
                    // Full mode
                    Console.SetMode(0);
                }
            ...
            public void LoadConsole()
            {
                if (Console == null)
                {
                    try
                    {
                        Console = new Core.Console.Controller();
                        Console.Initialize();
                    }
                    catch
                    {
            ...
            public void LoadTheme()
            {
                if (Theme == null)
                {
                    ...
                    {
                        Update(-1, "Exception [!]  <Module.Theme.Controller> not loaded");
                    }
                }
            }
            public void LoadSecurity()
            {
                if (Security == null)
                {
                    try
                    {
                        Security = new Windows.Security.Controller(string.Format("{0}[{1}]", DisplayName(), CurrentVersion()));

                        Update(0, "[+] <Module.Security.Controller> [Administrative privileges]");
                    }
                    catch
                    {
                        Update(-1, "Exception [!] <Module.Security.Controller> failed to get [Administrative privileges]");
                    }
                }
            }
            public void LoadToast()
            {
                if (Toast == null)
                {
                    try
                    {
                        ...
                        Update(0, "[+] <Module.Toast.Controller>");
                    }
                    catch
                    {
                        Update (-1, "Exception [!] <Module.Toast.Controller> failed to load");
                    }
                }
            }
            public void LoadHardware()
            {
                if (Hardware == null)
                {
                    try
                    {
                        ...
                        Update(0, "[+] <Module.Hardware.Controller>");
                    }
                    catch
                    {
                        Update (-1, "Exception [!] <Module.Hardware.Controller> failed to load");
                    }
                }
            }
            public void GetNetwork()
            {
                if (Network == null)
                {
                    try
                    {
                        ...
                        Update(1, "[+] <Module.Network.Controller>");
                    }
                    catch
                    {
                        Update(-1, "[+] <Module.Network.Controller> failed to load");
            ...
            public void GetProcess()
            {
                if (Process == null)
                {
                    try
                    {
                        Process = new Diagnostics.ProcessController();

                        Update(1, "[+] <Module.Process.Controller>");
                    }
                    catch
                    {
                        Update(-1, "Exception [!] <Module.Process.Controller> failed to load");
                        throw new Exception(Console.Last().Status);
                    }
                }
            }
            public void GetThread()
            {
                if (Thread == null)
                {
                    try
                    {
                        ...
                        Update(1, "[+] <Module.Thread.Controller>");
                    }
                    catch
                    {
                        Update(-1, "Exception [!] <Module.Thread.Controller> failed to load");
                        throw new Exception(Console.Last().Status);
                    }
                }
            }
            public void GetHost()
            {
                if (Host == null)
                {
                    try
                    {
                        ...
                        Update(0,"[+] <Module.Host.Controller(" + Host.Platform + ")>");
                    }
                    catch
                    {
                        Update(-1, "Exception [!] <Module.Host.Controller> failed to load");
                        throw new Exception(Console.Last().Status);
                    }
                }
            }
			public void GetRegistryKey()
            {
                if (Registry == null)
                {
                    try
                    {
                        Registry = new RegistryKeyController();

                        Update(0, "[+] <Module.RegistryKey.Controller>");
                    }
                    catch
                    {
                        Update(-1, "Exception [!] <Module.RegistryKey.Controller> failed to load");
                        throw new Exception(Console.Last().Status);
                    }
                }
            }
            public void AssignRegistryKey()
            {
                if (Registry != null)
                {
                    try
                    {
                        ...
                        if (Microsoft.Win32.Registry.GetValue(fullname.Replace("HKLM:","HKEY_LOCAL_MACHINE"), "", null) != null)
                            Registry.Assign(fullname);
                        else
                            Registry.Assign(fullname, DefaultTemplate());
                        ...
                        Update(0,"[+] <Module.RegistyKey.Controller> [Assigned]");
                    }
                    catch
                    {
                        Update(-1, "Exception [!] <Module.RegistryKey.Controller> [Not assigned]");
                        throw new Exception(Console.Last().Status);
                    }
                }
            }
            public void GetRoot()
            {
                if (Root == null)
                {
                    try
                    {
                        ...
                        Update(0, "[+] <Module.Root.Controller>");
                    }
                    catch
                    {
                        Update(-1, "Exception [!] <Module.Root.Controller> failed to load");
                        throw new Exception(Console.Last().Status);
                    }
                }
            }
            public void AssignRoot()
            {
                if (Root != null)
                {
                    try
                    {
                        Root.Assign(Registry.GetTemplate());   

                        Update(0,"[+] <Module.Root.Controller> [Assigned]");
                    }
                    catch
                    {
                        Update(-1, "Exception [!] <Module.Root.Controller> [Not assigned]");
                        throw new Exception(Console.Last().Status);
                    }
                }
            }
            public void GetManifest()
            {
                if (Manifest == null)
                {
                    try
                    {
                        Manifest = new ManifestController();

                        Update(0,"[+] <Module.Manifest.Controller>");
                    }
                    catch
                    {
                        Update(-1, "Exception [!] <Module.Manifest.Controller> failed to load");
                        throw new Exception(Console.Last().Status);
                    }
                }                
            }
            public void AssignManifest()
            {
                if (Manifest != null)
                {
                    try
                    {
                        Manifest.Assign(Registry.GetTemplate());

                        // Load manifest
                        LoadManifest();

                        Update(0,"[+] <Module.Manifest.Controller> [Assigned]");
                    }
                    catch
                    {
                        Update(-1, "Exception [!] <Module.Manifest.Controller> [Not assigned]");
                        throw new Exception(Console.Last().Status);
                    }
                }
            }
            ...
            {
                // report everything in the manifest like older version
                Update(0, "");
                Update(0, "-".PadRight(102, '-'));

                Manifest.AddFolder(type, name);
                Update(1, Manifest.LastStatus());
            ...
            private void LoadManifest()
            {
                ...
                // Graphics
                LoadManifestType(2, "Graphics", LoadManifestGraphic());
            }
            public void Install()
            {
                // Determine whether it is already installed
