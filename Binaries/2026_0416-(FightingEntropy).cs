...
namespace FightingEntropy
{
    ...
    namespace Diagnostics
    {
        ...
        public class ProcessTemplate
        {
            ...
            public ProcessObject Start()
            {
                Process p = Process.Start(Build());
                Instance = new ProcessObject(0, p.Id);
                return Instance;
            }
            public bool Stop()
            {
        ...
        public class ProcessObject
        {
            ...
            public ProcessObject(bool flag, uint index, string name, int pid)
            {
                Index = index;
                Name  = name;
                Id    = pid;
            ...
            public ProcessObject(uint index, Process process)
            {
                Index = index;
            ...
            public ProcessObject(uint index, int pid)
            {
                Index = index;
                Id    = pid;
                Refresh();
        ...
        {
            public List<ProcessTemplate> Template;
            public List<ProcessObject>    Process;
            ...
            {
                Template = new List<ProcessTemplate>();
            }
            public void Clear()
            {
                Process = new List<ProcessObject>();
            }
            public void Refresh()
            {
                Clear();
                ...
                foreach (System.Diagnostics.Process process in System.Diagnostics.Process.GetProcesses())
                {
                    try
                    {
                        if (!SafeCanQueryProcess(process))
                            continue;
                        
                        Process.Add(new ProcessObject((uint)Process.Count, process));
                    }
                    catch
                    {
                        string name = "";
                        ...
                        Process.Add(new ProcessObject(false, (uint)Process.Count, name, id));
                    }
                }
            }
            public List<ProcessObject> List()
            {                
                Process.Sort(delegate(ProcessObject a, ProcessObject b)
                {
            ...
            public static ProcessObject FromPid(int pid)
            {
                return new ProcessObject(0, pid);
            }
            public static ProcessObject FromHwnd(IntPtr hwnd)
            {
                if (!IsWindow(hwnd))
                    return null;
                ...
                return new ProcessObject(0, (int)pid);
            }
    ...
    namespace Hardware
    {
        ...
        namespace Wlan
        {
            ...
            {		
                ...
                public static extern uint WlanGetProfileList(IntPtr client, [MarshalAs(UnmanagedType.LPStruct)] Guid id, IntPtr reserved, out IntPtr profile);

                [DllImport("wlanapi.dll", EntryPoint = "WlanGetProfile")]
                public static extern uint WlanGetProfile(IntPtr client, [MarshalAs(UnmanagedType.LPStruct)] Guid id, [MarshalAs(UnmanagedType.LPWStr)] string name, IntPtr reserved, [MarshalAs(UnmanagedType.LPWStr)] out string xml, ref uint flags, out uint access);

                [DllImport("wlanapi.dll", EntryPoint = "WlanDeleteProfile")]
                public static extern uint WlanDeleteProfile(IntPtr client, [MarshalAs(UnmanagedType.LPStruct)] Guid id, [MarshalAs(UnmanagedType.LPWStr)] string name, IntPtr reserved);

                [DllImport("wlanapi.dll", EntryPoint = "WlanSetProfile", SetLastError = true, CharSet = CharSet.Unicode)]
                public static extern uint WlanSetProfile(IntPtr client, ref Guid id, uint flags, IntPtr xml, IntPtr all, bool Overwrite, IntPtr reserved, ref IntPtr reason);
                ...
                public static extern uint WlanGetAvailableNetworkList(IntPtr client, [MarshalAs(UnmanagedType.LPStruct)] Guid id, uint flags, IntPtr reserved, out IntPtr list);
    ...
    namespace Module
    {
        ...
        public class RegistryKeyController
        {
            ...
            public void Assign(string fullname, RegistryKeyTemplate template)
            {
                Initialize(fullname);
                Template       = template;
                ReadTemplate();
            ...
            private void Initialize(string fullname)
            {
            ...
            public static string GetHivePath(string pspath)
            {
                ...
                switch (pspath)
                {
                    case string s when Regex.IsMatch(s, "^HKLM:", RegexOptions.IgnoreCase):
                        result = s.Replace("HKLM:","HKEY_LOCAL_MACHINE");
                        break;
                    
                    case string s when Regex.IsMatch(s, "^HKCU:", RegexOptions.IgnoreCase):
                        result = s.Replace("HKCR:","HKEY_CURRENT_USER");
                        break;

                    case string s when Regex.IsMatch(s, "^HKU:", RegexOptions.IgnoreCase):
                        result = s.Replace("HKU:","HKEY_USERS");
                        break;
                    
                    case string s when Regex.IsMatch(s, "^HKCR:", RegexOptions.IgnoreCase):
                        result = s.Replace("HKCR:","HKEY_CLASSES_ROOT");
                        break;

                    case string s when Regex.IsMatch(s, "^HKCC:", RegexOptions.IgnoreCase):
                        result = s.Replace("^HKCC:","HKEY_CURRENT_CONFIG");
                        break;
                }
            ...
            public static bool PathExists(string fullname)
            {
                ...
                if (fullname.StartsWith("HKLM:\\"))
                    fullname = "HKEY_LOCAL_MACHINE" + fullname.Substring(5);
                else if (fullname.StartsWith("HKCU:\\"))
                    fullname = "HKEY_CURRENT_USER" + fullname.Substring(5);
                else if (fullname.StartsWith("HKCR:\\"))
                    fullname = "HKEY_CLASSES_ROOT" + fullname.Substring(5);
                else if (fullname.StartsWith("HKU:\\"))
                    fullname = "HKEY_USERS" + fullname.Substring(4);
                else if (fullname.StartsWith("HKCC:\\"))
                    fullname = "HKEY_CURRENT_CONFIG" + fullname.Substring(6);
                ...
                return root.OpenSubKey(sub) != null;
            }
        ...
        public class RootController
        {
            ...
            public void Assign(RegistryKeyTemplate template)
            {
                Registry = Designate(0, 0, "Registry" , template.Registry);
                Resource = Designate(1, 0, "Resource" , template.Resource);
                Module   = Designate(2, 0, "Module"   , template.Module);
                File     = Designate(3, 1, "File"     , template.File);
                Manifest = Designate(4, 1, "Manifest" , template.Manifest);
                Shortcut = Designate(5, 1, "Shortcut" , template.Shortcut);
            }
            private RootProperty Designate(uint index, uint mode, string name, string fullname)
            {
        ...
        public class ManifestController
        {
            ...
            public string              Source;
            public string            Resource;
            public List<ManifestEntry>  Entry;
            ...
            private void Add(ManifestEntry entry)
            {
            ...
            {
                ManifestEntry entry = new ManifestEntry((uint)Entry.Count, type, name);

                AssignEntry(entry);
            }
            public void AddFile(uint type, string name, string hash)
            {
                ManifestEntry entry = new ManifestEntry((uint)Entry.Count, type, name, hash);

                AssignEntry(entry);
            }
            private void AssignEntry(ManifestEntry entry)
            {
            ...
            public void Refresh()
            {
                foreach (ManifestEntry entry in Entry)
                {
                    entry.Check();
                }
            }
        ...
        public class Controller
        {
            ...
            public Theme.Controller                Theme;
            public Toast.Controller                Toast;
            public Security.Controller          Security;
            public Hardware.Controller          Hardware;
            public Diagnostics.ProcessController Process;
            public Network.Controller            Network;
            public Thread.Controller              Thread;
            ...
            private void Initialize(EngineIntrinsics context)
            {
                ...
                // Get Security.Principal/Identity/Certificates
                GetSecurity();

                // Get Toast.Controller engine
                GetToast();
            ...
            public void GetSecurity()
            {
                if (Security == null)
                {
                    try
                    {
                        Security = new Security.Controller(string.Format("{0}[{1}]", DisplayName(), CurrentVersion()));
            ...
            public void GetHardware()
            {
                if (Hardware == null)
                {
                    try
                    {
                        Hardware = new Hardware.Controller();
