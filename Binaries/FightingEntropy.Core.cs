...
namespace FightingEntropy
{
    ...
    namespace Core
    {
        ...
        namespace Interop
        {
            ...
            using Core.Platform.Security.Certificate;

            public interface ISecurity
            {
                Identifier         Domain { get; }
                List<Store>         Store { get; }
                Context           Context { get; }
                Credential     Credential { get; }
                List<Reference> Reference { get; }
                void Refresh();
                void SetCredential(Credential credential);
                void SetCredential(string username);
            }

            public abstract class Security : Provider, ISecurity
            {
                ...
                public void GetReference()
                {
                    ...
                    foreach (string line in output)
                    {
                        ...
                        Reference.Add(new Reference((uint)Reference.Count, split[1].Trim(), split[0].Trim()));
                    }
                }
                ...
                public static ProtocolMode GetProtocolMode(uint mode)
                {
                    switch (mode)
                    {
                        case 48:    return ProtocolMode.Ssl3;
                        case 192:   return ProtocolMode.Tls10;
                        case 768:   return ProtocolMode.Tls11;
                        case 3072:  return ProtocolMode.Tls12;
                        case 12288: return ProtocolMode.Tls13;
                        default:    return ProtocolMode.None;
                    }
                }
                public static AuthenticationType GetAuthType(string value)
                {
                    if (string.IsNullOrEmpty(value))
                        return AuthenticationType.Unspecified;
                    ...
                    switch (value.ToLower())
                    {
                        ...
                        default          : return AuthenticationType.Unspecified;
                    }
                }
                public static AccountType GetAccountType(Identifier sid)
                {
                    ...
                    if (sid.Sid.StartsWith("S-1-5-18")) return AccountType.LocalSystem;
                    if (sid.Sid.StartsWith("S-1-5-19")) return AccountType.LocalService;
                    if (sid.Sid.StartsWith("S-1-5-20")) return AccountType.NetworkService;
                    if (sid.Sid.StartsWith("S-1-5-21")) return AccountType.User;
                    if (sid.Sid.StartsWith("S-1-5-32")) return AccountType.BuiltinGroup;
