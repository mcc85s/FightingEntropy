...
namespace FightingEntropy
{
    ...
    namespace Core
    {
        ...
        namespace Platform
        {
            ...
            namespace Process
            {
                ...
                public class Entry
                {
                    ...
                    public Entry() { }
                    public Entry(int id, string name, string path, bool done, int ssid, ulong work, ulong vmem, ulong paged, ulong non, TimeSpan cpu, IntPtr win)
                    {
                        Id             = id;
                        Name           = name;
                        Path           = path;
        ...
        namespace Interop
        {
            ...
            {
                List<Platform.Process.Entry>       Entry { get; }
                List<Platform.Process.Template> Template { get; }
                Platform.Process.Entry           Current { get; }
                void Clear();
                void ClearTemplate();
                void Refresh();
                ...
                Core.Platform.Process.Template GetTemplate(uint index);
                Core.Platform.Process.Template GetTemplate(string displayname);
            ...
            public abstract class Process : Core.Platform.Process.Provider, IProcess
            {
                ...
                public virtual void ClearTemplate()
                {
                    Template.Clear();
                }
                public abstract void Refresh();
                public abstract void Start(uint index);
                public abstract void Start(string displayname);
                public abstract void Stop(uint index);
                public abstract void Stop(string displayname);
                ...
                public virtual void Remove(string displayname)
                {
                    var template = GetTemplate(displayname);
                    if (template != null)
                    {
                        Template.Remove(template);
                        Rerank();
                ...
                public virtual Core.Platform.Process.Entry GetProcess(int id)
                {
                    return Entry.FirstOrDefault(x => x.Id == id);
                }
                public virtual Core.Platform.Process.Entry GetProcess(string name)
                {
                    return Entry.FirstOrDefault(x => x.Name == name);
                }
                public virtual Core.Platform.Process.Template GetTemplate(uint index)
                {
                    return Template.FirstOrDefault(x => x.Index == index);
                }
                public virtual Core.Platform.Process.Template GetTemplate(string displayname)
                {
                    return Template.FirstOrDefault(x => x.DisplayName == displayname);
                }
                public virtual void SetCurrent(int id)
                {
                    Current = GetProcess(id);
                }
                public void Create(
                    string displayname,
                    string file, 
                    ...
                    bool  admin = false)
                {
                    ...
                    var t                   = new Core.Platform.Process.Template
                    {
                        Index               = (uint)Template.Count,
                        DisplayName         = displayname,
                        FileName            = file,
                        Arguments           = args,
                        WorkingDirectory    = dir,
            ...
            public abstract class Security : Core.Platform.Security.Provider, ISecurity
            {
                ...
                public abstract void ResolveAccountList();
            ...
            public abstract class Configuration : Platform.Configuration.Provider, IConfiguration
            {
                ...
                public string CurrentVersion() => "2026.8.0";
