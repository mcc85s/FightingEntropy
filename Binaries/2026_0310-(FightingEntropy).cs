...
namespace FightingEntropy
{
    ...
    namespace Thread
    {
        ...
        public class Controller
        {
            ...
            public List<WorkerParam>              WorkerParams { get; set; }
            public Controller()
            {
            ...
            public void InitCollections()
            {
                ...
                WorkerParams = new List<WorkerParam>();
            }
            ...
            public void AddWorkerParams(List<KeyValuePair<string,string>> items)
            {
                WorkerParams.Clear();
                ...
                for (int i = 0; i < items.Count; i++)
                {
                    ...
                    wp.Name = items[i].Key;
                    wp.Type = items[i].Value;
                    WorkerParams.Add(wp);
                }
            }
            private string BuildParamBlock()
            {
                ...
                {
                    WorkerParam wp = WorkerParams[i];

                    if (wp.Type != null && wp.Type.Length > 0)
                        sb.Append("[" + wp.Type + "]");

                    sb.Append("$" + wp.Name);

                    if (i < WorkerParams.Count - 1)
                        sb.Append(", ");
                }
            ...
            private string BuildBootstrapBlock()
            {
                ...
                for (int i = 0; i < SessionState.Count; i++)
                {
                    ...
                    switch (item.Type)
                    {
                        ...
                        case SessionStateType.Variable:
                            sb.AppendLine("# Define variable: " + item.Name);
                            sb.AppendLine("Set-Variable -Name '" + item.Name + "' -Value ([System.Text.Encoding]::UTF8.GetString($" + item.Name + "))");
                            sb.AppendLine();
                            break;

                        case SessionStateType.Function:
                            sb.AppendLine("# Define function: " + item.Name);
                            sb.AppendLine("Set-Item -Path ('Function:\\' + '" + item.Name + "') -Value ([System.Text.Encoding]::UTF8.GetString($" + item.Name + "))");
                            sb.AppendLine();
                            break;

                        case SessionStateType.Script:
                            // IMPORTANT:
                            // Inline scripts are NOT the worker script.
                            // They are environment scripts.
                            sb.AppendLine("# Inline script: " + item.Name);
                            sb.AppendLine("Invoke-Expression ([System.Text.Encoding]::UTF8.GetString($" + item.Name + "))");
                            sb.AppendLine();
                            break;
                    }
                }
            ...
            public string BuildWorkerScript()
            {
                ...
                // 3. decode and run the worker script
            ...
            new System.Text.RegularExpressions.Regex(@"param\s*\(([^)]*)\)", System.Text.RegularExpressions.RegexOptions.IgnoreCase);
            public static List<KeyValuePair<string,string>> ExtractParams(string scriptText)
            {
                List<KeyValuePair<string,string>> list =new List<KeyValuePair<string,string>>();

                System.Text.RegularExpressions.Match m = ParamRegex.Match(scriptText);
                if (!m.Success)
                    return list;
                ...
                for (int i = 0; i < parts.Length; i++)
                {
                    ...
                    {
                        type = p.Substring(bracketStart + 1, bracketEnd - bracketStart - 1).Trim();
                        p = p.Substring(bracketEnd + 1).Trim();
                    }

                    if (p.StartsWith("$"))
                        name = p.Substring(1).Trim();
                    else
                        name = p.Trim();

                    if (name.Length > 0)
                        list.Add(new KeyValuePair<string,string>(name, type));
                }
