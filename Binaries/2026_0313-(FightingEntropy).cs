        ...
        public class Provider
        {
            ...
            public void Collect()
            {
                ...
                if (!File.Exists(logPath))
                    throw new FileNotFoundException("EVTC log file not found", logPath);
                ...
                try
                {
                    ...
                    using (var reader = new EventLogReader(query))
                    {
                        ...
                        while (true)
                        {
                            ...
                            if (estTotal > 0)
                            {
                                double pct = (estTotal > 0) ? ((double)count / estTotal) * 100.0 : 0;

                                msg = string.Format("Processing [~] {0} {1:N2}% ({2}/{3})", DisplayName, pct, count, estTotal);
            ...
            private void CollectEventLogs()
            {
                ...
                while (tc.ActiveJobs() > 0 || !tc.Messages.IsEmpty)
                {
                    ...
                    if (tc.HeartbeatDue(60))
                    {
                        ...
                        string hb = string.Format("Heartbeat [~] {0:00.00}% ({1:N0}/{2:N0} records)", pct, globalCurrent, globalTotal);
