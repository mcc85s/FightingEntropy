        ...
        namespace Process
        {
            public class Manager : Core.Interop.Process
            {
                ...
                public Core.Platform.Process.Entry ParseEntry(int pid)
                {
                    try
                    {
                        var stat_   = $"/proc/{pid}/stat";
                        var cmd_    = $"/proc/{pid}/cmdline";
                        var status_ = $"/proc/{pid}/status";

                        if (!System.IO.File.Exists(stat_))
                            return null;

                        var   stat = System.IO.File.ReadAllText(stat_).Split(' ');
                        var    cmd = System.IO.File.Exists(cmd_) ? System.IO.File.ReadAllText(cmd_).Replace('\0', ' ') : string.Empty;
                        var status = System.IO.File.Exists(status_) ? System.IO.File.ReadAllLines(status_) : Array.Empty<string>();

                        long  vmrss = GetMem(status, "VmRSS");
                        long vmsize = GetMem(status, "VmSize");
                        long vmswap = GetMem(status, "VmSwap");

                        long  utime = long.Parse(stat[13]);
                        long  stime = long.Parse(stat[14]);
                        long  ticks = 100; // Linux default
                        double  cpu = (utime + stime) / (double)ticks;

                        int     uid = GetUid(status);

                        return new Core.Platform.Process.Entry(
                            pid, 
                            stat[1].Trim('(', ')'), 
                            cmd, 
                            false, 
                ...
                public long GetMem(string[] status, string key)
                {
                    var line = status.FirstOrDefault(x => x.StartsWith(key));
                    if (line == null) return 0;

                    var parts = line.Split(':')[1].Trim().Split(' ');
                    return long.Parse(parts[0]) * 1024;
                }
                ...
                public override void Start(uint index)
                {
                    var t = GetTemplate(index);
                    if (t == null)
                        throw new ArgumentException($"Exception [!] Invalid index: {index}");

                    StartTemplate(t);
                }
                public override void Start(string displayname)
                {
                    var t = GetTemplate(displayname);
                    if (t == null)
                        throw new ArgumentException($"Exception [!] Invalid name: {displayname}");

                    StartTemplate(t);
                }
                public override void Stop(uint index)
                {
                    var t = GetTemplate(index);
                    if (t?.Process == null)
                        return;
                ...
                public override void Stop(string displayname)
                {
                    var t = GetTemplate(displayname);
                    if (t?.Process == null)
                        return;
                ...
                public void StartTemplate(Core.Platform.Process.Template template)
                {
                    var psi = new System.Diagnostics.ProcessStartInfo
                    {
                        ...
                        UseShellExecute        = template.UseShellExecute,
                        RedirectStandardOutput = template.RedirectStdOut,
                        RedirectStandardError  = template.RedirectStdErr,
                        RedirectStandardInput  = template.RedirectStdIn,
                        CreateNoWindow         = template.CreateNoWindow,
                    };
                    ...
                    var entry = ParseEntry(proc.Id) ?? new Core.Platform.Process.Entry(
                        proc.Id,
                        proc.ProcessName,
                        template.FileName,
                        proc.HasExited,
                    ...
                    Entry.Add(entry);
                    Current          = entry;
                    template.Process = entry;
                }
                public void StopTemplate(Core.Platform.Process.Template t)
                {
        ...
        namespace Security
        {
            ...
            public sealed class Manager : Core.Interop.Security
            {
                ...
                public void Initialize()
                {
                    ...
                    if (uid == 0)
                    {
                        System.Console.WriteLine("Granted [+] Administrative access");
                    }
                    else
                    {
                        if (!Regex.Split(RunProcess("id", "-nG"), @"\s+").Any(g => Regex.IsMatch(g,"^(sudo|wheel)$")))
                        {
                            throw new Exception("Denied [!] Administrative access");
                        }

                        if (Credential == null || Credential.Password == null)
                        {
                            System.Console.WriteLine($"Requesting [~] Administrative access");

                            while (!valid && limit < 3)
                            {
                                System.Console.WriteLine($"Attempt ({limit + 1 }/3)");

                                SetCredential(username);
                                valid = VerifySudoCredential();

                                if (!valid)
                                    System.Console.WriteLine("Exception [!] Invalid password");
                ...
                public override void ResolveDomain()
                {
                    ...
                    try
                    {
                        ...
                        // 2. Get (domain/list)
                        output = RunProcess(xsssctl.Name, "domain-list", true);
                        ...
                        output            = RunProcess(xsssctl.Name, $"domain-status {domain}", true);
                        ...
                        try
                        {
                            RunProcess(xkinit.Name, $"-k {principal}", true);
                        }
                        catch
                        {
                        ...
                        output = RunProcess(xklist.Name, null, true);
                        ...
                        output         = RunProcess(xldapsearch.Name, arglist, true);
                ...
                public override void ResolveAccountList()
                {
                    ...
                    if (Domain != null && !Dependency.Entry.Any(e => e.Exists == false))
                    {
                        ...
                        string output = RunProcess(ldapsearch.Fullname, args, true);
                        if (string.IsNullOrWhiteSpace(output))
                        {
                ...
                public List<Identifier> ResolveAccountGroups(Account account)
                {
                    ...
                    foreach (var groupName in RunProcess("id", $"-nG {account.Username}", true).Split(' ', StringSplitOptions.RemoveEmptyEntries))
                    {
                        string[] groupInfo = RunProcess("getent", $"group {groupName}", true).Split(':', StringSplitOptions.RemoveEmptyEntries);
                ...
                private bool VerifySudoCredential()
                {
                ...
                public string RunProcess(string name, string arguments, bool sudo = false)
                {
                    ...
                    {
                        if (Credential == null || Credential.Password == null)
                            throw new Exception("Exception [!] Sudo execution requires credentials");
                ...
                {
                    string output = RunProcess("sudo", $"-n -l -U {username}");
