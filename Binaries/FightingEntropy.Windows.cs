...
namespace FightingEntropy
{
    namespace Windows
    {
        ...
        namespace Interop
        {
            ...
            {
                public override Core.Interop.ISecurity           Security { get; }
                public override Core.Interop.IConfiguration Configuration { get; }
                public override Core.Interop.IFileSystem       FileSystem => throw new NotImplementedException();
                public override Core.Interop.IProcess             Process => throw new NotImplementedException();
                public override Core.Interop.IService             Service => throw new NotImplementedException();
                public override Core.Interop.ICommand             Command => throw new NotImplementedException();
                public override Core.Interop.INetwork             Network => throw new NotImplementedException();
                public override Core.Interop.IHardware           Hardware => throw new NotImplementedException();
                public Controller()
                {
        ...
        namespace Security
        {
            public class Identifier : Core.Platform.Security.Identifier
            {
                ...
                public Identifier(int uid, int? gid = null) : base(uid, gid) { }
                ...
                {
                    return (Sid != null) ? new SecurityIdentifier(Sid) : null;
                }
            ...
            {
                public Role() { }
                public Role(string name, string sid, IEnumerable<string> members)
                {
                    Name    = name;
                    Sid     = sid;
                    Members = members?.ToList() ?? new List<string>();

                    Gid     = null;
                    Rid     = null;

                    if (!string.IsNullOrWhiteSpace(sid) && Regex.IsMatch(sid, @"^S-\d+(-\d+){1,14}$"))
                    {
                        var parts = sid.Split('-');
            ...
            {
                public Principal() { }
                public Principal(Account account, Identifier identifier, List<Role> roles, List<Claim> claims)
                {
                    Account    = account;
            ...
            public sealed class Manager : Core.Interop.Security
            {
                ...
                // Platform.Security.Context                     Context { get; }
                ...
                protected override void ResolveDomain()
                {
                    ...
                    if (wi?.User?.AccountDomainSid != null)
                        Domain = new Identifier(wi.User.AccountDomainSid.Value);
                }
                public Certificate.Store CreateStore(string displayname, string fullname)
                {
                ...
                public Identity CreateIdentity(WindowsIdentity wi)
                {
                    ...
                    {
                        Name            = wi.Name,
                        Domain          = wi.User.AccountDomainSid?.Value ?? "",
                        Id              = wi.User.Value, // SID
                        IsAdministrator = wp.IsInRole(WindowsBuiltInRole.Administrator),
                        IsAuthenticated = wi.IsAuthenticated,
                        Role            = wi.Groups.Select(g => new Core.Platform.Security.Role
                        {
                ...
                public Principal CreatePrincipal(Identity identity)
                {
                    return new Principal
                    {
                        Account    = null,
                        Identifier = new Identifier(identity.Id),
                        Role       = identity.Role,
                        Claim      = identity.Role.Select(c => new Core.Platform.Security.Claim { Type  = "role", Value = c }).ToList()
                    };
                }
                protected override void ResolveStoreList()
                {
                    ...
                    Store.Add(CreateStore(new X509Store(StoreName.My,   StoreLocation.LocalMachine)));
                    Store.Add(CreateStore(new X509Store(StoreName.Root, StoreLocation.LocalMachine)));
                    Store.Add(CreateStore(new X509Store(StoreName.My,   StoreLocation.CurrentUser)));
                    Store.Add(CreateStore(new X509Store(StoreName.Root, StoreLocation.CurrentUser)));
                }
                ...
                protected override void ResolveContext()
                {
                    ...
                    Identity   identity = CreateIdentity(wi);
                    Principal principal = CreatePrincipal(identity);
                    ...
                    Context             = new Context
                    {
                        ...
                        UserId          = identity.Id,
                        GroupId         = null // Windows does not use GID
                    };
                }
        ...
        namespace Configuration
        {
            ...
            public class Manager : Core.Interop.Configuration
            {
                ...
                public Manager() : base()
                {
                    Type = Core.Platform.Configuration.Type.Registry;
                    Clear();
                }
                public override List<Version> GetVersions()
                {
                    ...
                    string branch = ConfigurationPath().Replace("HKLM:\\","");

                    using var baseKey = Hive.OpenSubKey(branch);
                    if (baseKey == null)
                        return versions;

                    foreach (string subKeyName in baseKey.GetSubKeyNames())
                    {
                        if (Regex.IsMatch(subKeyName, @"^\d{4}\.\d{1,}\.\d{1}$"))
                        {
                            try
                            {
                                versions.Add(Version.Parse(subKeyName));
                            }
                            catch
                            {
                ...
                public override void Initialize()
                {
                    ...
                    RegistryKey current = Hive;

                    foreach (string part in parts)
                    {
                        current = current.OpenSubKey(part, true) ?? current.CreateSubKey(part, true);
                    }
                ...
                public override void Initialize(string fullname)
                {
                    ...
                    Branch = System.IO.Path.GetDirectoryName(fullname).Replace("HKLM:\\","");
                    Root   = Branch;
                    Path   = fullname;
                    Drive  = Root;
                ...
                public override void Check()
                {
                    ...
                    if (Hive == null || string.IsNullOrEmpty(Branch) || string.IsNullOrEmpty(Name))
                        return;
                    ...
                    Exists = true;
                    Size   = new Format.ByteSize("Registry", (ulong)key.ValueCount);
                }
                ...
                public override void Write()
                {
                    ...
                    using var parent = Hive.OpenSubKey(Branch, true);
                ...
                public override void Create()
                {
                    Check();
                    if (Exists)
                        throw new Exception("Registry path already exists");

                    using var parent = Hive.OpenSubKey(Branch, true)
                                    ?? Hive.CreateSubKey(Branch, true);

                    parent.CreateSubKey(Name, writable: true);
                ...
                {
                    return $"HKLM:\\Software\\Policies\\{CompanyName()}\\{ProjectName()}";
                }
                public Property CreateProperty(uint index, string name, object value)
                {
                ...
                public override void Resolve()
                {
                    ...
                    using (var baseKey = Hive.OpenSubKey(branch))
                    {
                        if (baseKey != null)
                        {
                            foreach (string subKeyName in baseKey.GetSubKeyNames())
                            {
                                if (Regex.IsMatch(subKeyName, @"^\d{4}\.\d{1,}\.\d{1}$"))
                                {
                                    try { versions.Add(Version.Parse(subKeyName)); }
                                    catch { }
                                }
                            }
                        }
                    ...
                    string name    = latest.ToString();

                    using (var parent = Hive.OpenSubKey(branch, true) ?? Hive.CreateSubKey(branch, true))
                    {
                        parent.CreateSubKey(name, writable: true);
                    }
        ...
        namespace Registry
        {
            ...
            {
                private Core.Module.Template Template;
                ...
                public void Assign(string fullname, Core.Module.Template template)
                {
                    Initialize(fullname);
                    Template = template;
                    ReadTemplate();
                ...
                {
                    Clear();
                    FieldInfo[] fields = typeof(Core.Module.Template).GetFields(BindingFlags.Public | BindingFlags.Instance);
                ...
                {
                    FieldInfo[] fields = typeof(Module.Template).GetFields(BindingFlags.Public | BindingFlags.Instance);
